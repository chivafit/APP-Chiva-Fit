...
async function main() {
  ...
  const toRowObject = (row, rowNum) => {
    ...
    for (const [csvCol, stgCol] of Object.entries(map)) {
      ...
      if (
        stgCol.includes('valor') ||
        stgCol.includes('preco') ||
        stgCol.includes('desconto') ||
        stgCol.includes('frete') ||
        stgCol.includes('outras_despesas') ||
        stgCol === 'quantidade'
      ) {
  ...
  for (let i = 0; i < dataRows.length; i += batchSize) {
    ...
    process.stdout.write(
      `inserted ${Math.min(i + batchSize, dataRows.length)}/${dataRows.length}\n`,
    );
  }
