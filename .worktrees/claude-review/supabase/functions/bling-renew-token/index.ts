...
async function ensureLockRowExists(supabase: SupabaseClient, lockKey: string) {
  ...
  try {
    await supabase.from('configuracoes').insert([
      {
        chave: lockKey,
        valor_texto: JSON.stringify({ locked: false, by: null, at: null }),
        updated_at: nowIso(),
      },
    ]);
  } catch (_e) {}
...
async function acquireRefreshLock(supabase: SupabaseClient, lockKey: string, requestId: string) {
  ...
  while (Date.now() < deadline) {
    ...
    const curVal = String(
      current?.data?.valor_texto ?? JSON.stringify({ locked: false, by: null, at: null }),
    );
    const parsed = parseLock(curVal);
    const lockAtMs = parsed.at ? new Date(String(parsed.at)).getTime() : 0;
    const isStale =
      parsed.locked && (!lockAtMs || isNaN(lockAtMs) || Date.now() - lockAtMs > staleMs);
...
serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      const cfgRefresh = String(
        (await getConfigValue(supabase, 'bling_refresh_token')) || '',
      ).trim();
    ...
    if (!currentRefreshToken) {
      throw new Error(
        'Missing refresh token (set BLING_REFRESH_TOKEN secret or store bling_refresh_token in configuracoes).',
      );
    }
        ...
        return json(
          {
            error: 'invalid_grant',
            message:
              'Refresh Token do Bling inválido/rotacionado. Reautorize o Bling nas Configurações.',
            reauthorize: true,
          },
          401,
        );
    ...
    try {
      await upsertConfigValue(supabase, 'bling_token_error', '');
    } catch (_e) {}
