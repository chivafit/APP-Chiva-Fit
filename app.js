...
try {
  ...
  console.warn('[init] localStorage indisponível — modo privado?');
}
...
let allTasks = safeJsonParse('crm_tasks', null) || [
  {
    id: 1,
    titulo: 'Reativar clientes VIP inativos (WhatsApp)',
    desc: 'Clientes VIP que não compram há +60 dias',
    prioridade: 'alta',
    status: 'pendente',
    cliente: '',
  ...
  {
    id: 2,
    titulo: 'Enviar campanha de reativação',
    desc: 'Segmento de inativos - template WhatsApp',
    prioridade: 'media',
    status: 'pendente',
    cliente: '',
    data: new Date().toISOString().slice(0, 10),
  ...
  {
    id: 3,
    titulo: 'Verificar pedidos pendentes no Bling',
    desc: 'Conferir status de entregas desta semana',
    prioridade: 'alta',
    status: 'em_andamento',
    cliente: '',
    data: new Date().toISOString().slice(0, 10),
    ...
    try {
      ...
      try {
        if (
          !supaAuthUnsub &&
          supaClient.auth &&
          typeof supaClient.auth.onAuthStateChange === 'function'
        ) {
...
try {
  (() => {
    mergeOrders();
    const now = new Date(),
      from = new Date(now.getFullYear(), now.getMonth() - 17, 1);
    ...
    const pm = now.getMonth() === 0 ? 12 : now.getMonth(),
      py = now.getMonth() === 0 ? y - 1 : y;
    const ca = document.getElementById('cmp-a'),
      cb = document.getElementById('cmp-b');
    if (ca) ca.value = `${y}-${m}`;
    if (cb) cb.value = `${py}-${String(pm).padStart(2, '0')}`;
    const ad = localStorage.getItem('crm_alertdays');
    if (ad) {
      const el = document.getElementById('alert-days');
      if (el) el.value = ad;
    }
    if (CID) {
      const el = document.getElementById('inp-cid');
      if (el) el.value = CID;
    }
    if (CSEC) {
      const el = document.getElementById('inp-csec');
      if (el) el.value = CSEC;
    }
    if (SHOP) {
      const el = document.getElementById('inp-shop');
      if (el) el.value = SHOP;
    }
    if (SHOPKEY) {
      const el = document.getElementById('inp-shopkey');
      if (el) el.value = SHOPKEY;
    }
...
function mergeOrders() {
  ...
  const nextCustomers = Object.values(buildCli(allOrders)).map((c) => {
    ...
    return {
      ...
      total_gasto: sc.ltv,
      status: sc.status,
      canal_principal: c.channels && c.channels.size ? [...c.channels][0] : '',
    };
  });
  ...
  (Array.isArray(yampiOrders) ? yampiOrders : []).forEach((yo) => {
    ...
    const doc = String(
      yo?.doc || yo?.cpfCnpj || yo?.contato?.cpfCnpj || yo?.contato?.numeroDocumento || '',
    ).trim();
  ...
  reconcileCarrinhosRecuperados().catch((e) =>
    console.warn('[reconcile carrinhos]', e?.message || e),
  );
  recomputeCarrinhosScoresAndPersist().catch((e) =>
    console.warn('[recompute carrinhos]', e?.message || e),
  );
}
...
function checkEstoqueCritico() {
  ...
  const hash = crit
    .map(
      (i) =>
        `${String(i?.id || '')}:${String(i?.nome || '')}:${Number(i?.estoque_atual ?? i?.estoque ?? 0) || 0}:${Number(i?.estoque_minimo ?? i?.minimo ?? 0) || 0}`,
    )
    .sort()
    .join('|');
    ...
    if (lastLogged !== today) {
      ...
      supaClient
        ...
        .insert({
          ...
          conteudo: {
            itens: crit.map((i) => ({
              id: i?.id,
              nome: i?.nome,
              estoque_atual: i?.estoque_atual ?? i?.estoque ?? 0,
              estoque_minimo: i?.estoque_minimo ?? i?.minimo ?? 0,
            })),
          },
        ...
        .catch((e) => {
          ...
          captureError(e, { context: 'insert_alerta_estoque' });
        });
    }
  }
}
...
function supaFnHeaders() {
  ...
  if (!anonKey)
    throw new Error('Supabase não configurado: informe a chave pública (anon) em Configurações.');
  if (!supaAccessToken) {
    throw new Error('Sessão do Supabase expirada. Faça login novamente.');
  }
  return {
...
async function handleLoginSubmit(e) {
  ...
  try {
    ...
    const canonicalEmail =
      email === 'admin' || email === 'admin@chivafit.com.br' ? 'admin@chivafit.com' : email;
    ...
    if (hasSupabase) {
      ...
      if (error || !data?.session?.access_token) {
        if (errEl)
          errEl.textContent = error?.message
            ...
            : 'Falha ao autenticar no Supabase (Auth).';
        return false;
      }
      ...
      if (!connected) {
        ...
        if (errEl)
          errEl.textContent =
            st && st.textContent.includes('⚠')
              ? st.textContent.replace('⚠ ', '')
              : 'Supabase não está conectado. Verifique a URL e a Chave.';
        if (btnEl) {
    ...
    if (!window.isSecureContext) {
      if (errEl)
        errEl.textContent =
          'Este login precisa de HTTPS (ou servidor local). Evite abrir o arquivo via file://';
    } else {
...
function enterApp(userEmail) {
  ...
  if (topbarAvatarEl)
    topbarAvatarEl.textContent = userEmail && userEmail !== 'admin@chivafit.com' ? '👤' : '🛡️';
...
function getCarrinhosAlertados() {
  try {
    return new Set(JSON.parse(localStorage.getItem(CARRINHO_ALERTA_KEY) || '[]'));
  } catch (_) {
    return new Set();
  }
}
function setCarrinhosAlertados(s) {
  try {
    localStorage.setItem(CARRINHO_ALERTA_KEY, JSON.stringify([...s].slice(-300)));
  } catch (_) {}
}

function checkCarrinhosQuentes() {
  const lista = (carrinhosAbandonados || [])
    .map(normalizeCarrinhoAbandonado)
    .filter((c) => c.checkout_id);
  const quentes = lista.filter(
    (c) =>
      !c.recuperado &&
      !c.perdido &&
      minutosDesdeIso(c.criado_em) != null &&
      minutosDesdeIso(c.criado_em) < CARRINHO_HOT_MIN,
  );
...
function exportCarrinhosCSV() {
  ...
  const list = (carrinhosAbandonados || [])
    ...
    .filter((c) => {
      ...
      if (
        ...
        (c.recuperado ||
          c.perdido ||
          minutosDesdeIso(c.criado_em) == null ||
          minutosDesdeIso(c.criado_em) >= 120)
      )
        return false;
    ...
    .map((c) => {
      ...
      const prodTxt = itens
        .slice(0, 5)
        .map((it) => String(it?.nome || it?.title || it?.descricao || it?.name || '').trim())
        .filter(Boolean)
        .join('; ');
      const pipeline = c.recuperado
        ? 'Recuperado'
        : c.perdido
          ? 'Perdido'
          : c.last_etapa_enviada
            ? 'Contatado'
            : 'Novo';
  ...
  toast(
    `✓ ${list.length} carrinho${list.length !== 1 ? 's' : ''} exportado${list.length !== 1 ? 's' : ''}!`,
    'success',
  );
}
...
function renderCarrinhosFunil(enrichedList) {
  ...
  if (!total) {
    el.innerHTML =
      '<div style="text-align:center;font-size:12px;color:var(--text-3);padding:12px">Sem dados.</div>';
    return;
  }
  ...
  const stages = [
    {
      ...
      label: 'Novo',
      color: '#64748b',
      count: list.filter((c) => !c.recuperado && !c.perdido && !c.last_etapa_enviada).length,
    },
    {
      id: 'ajuda',
      label: 'Ajuda enviada',
      color: '#3b82f6',
      count: list.filter((c) => !c.recuperado && !c.perdido && c.last_etapa_enviada === 'ajuda')
        .length,
    },
    {
      id: 'link',
      label: 'Link enviado',
      color: '#8b5cf6',
      count: list.filter((c) => !c.recuperado && !c.perdido && c.last_etapa_enviada === 'link')
        .length,
    },
    {
      id: 'incentivo',
      label: 'Incentivo',
      color: '#f59e0b',
      count: list.filter((c) => !c.recuperado && !c.perdido && c.last_etapa_enviada === 'incentivo')
        .length,
    },
    {
      id: 'recuperado',
      label: 'Recuperado ✓',
      color: '#10b981',
      count: list.filter((c) => c.recuperado).length,
    },
    {
      id: 'perdido',
      label: 'Perdido ✗',
      color: '#ef4444',
      count: list.filter((c) => c.perdido && !c.recuperado).length,
    },
  ];
  ...
  el.innerHTML =
    ...
    stages
      .map((s) => {
        ...
        return (
          '<div class="car-funil-row">' +
          '<div class="car-funil-label">' +
          escapeHTML(s.label) +
          '</div>' +
          '<div class="car-funil-bar-wrap"><div class="car-funil-bar-fill" style="width:' +
          barW +
          '%;background:' +
          s.color +
          '"></div></div>' +
          '<div class="car-funil-count">' +
          escapeHTML(String(s.count)) +
          '</div>' +
          '<div class="car-funil-pct">' +
...
function populateCarRespFilter() {
  ...
  const existentes = [
    ...new Set(
      (carrinhosAbandonados || []).map((c) => String(c.responsavel || '')).filter(Boolean),
    ),
  ];
...
function atribuirResponsavelCarrinho(checkoutId, responsavel) {
  ...
  if (idx < 0) return;
  carrinhosAbandonados[idx] = Object.assign(
    {},
    normalizeCarrinhoAbandonado(carrinhosAbandonados[idx]),
    { responsavel: responsavel || null },
  );
  safeSetItem('crm_carrinhos_abandonados', JSON.stringify(carrinhosAbandonados));
  if (supaConnected && supaClient)
    upsertCarrinhosAbandonadosToSupabase([carrinhosAbandonados[idx]]).catch(() => {});
}
...
async function openCarrinhoHistorico(checkoutId) {
  ...
  body.innerHTML =
    '<div style="text-align:center;padding:20px 0;color:var(--text-3);font-size:12px">Carregando...</div>';
  modal.style.display = '';

  if (!supaConnected || !supaClient) {
    body.innerHTML =
      '<div style="padding:16px 0;text-align:center;font-size:12px;color:var(--text-3)">Conexão com Supabase necessária para ver o histórico.</div>';
    return;
  }
  ...
  try {
    ...
    if (!display.length) {
      body.innerHTML =
        '<div style="padding:16px;text-align:center;font-size:12px;color:var(--text-3)">Nenhuma interação registrada. O histórico é criado ao enviar mensagens pelo CRM.</div>';
      return;
    }
...
function openClienteDrawer(clienteId) {
  try {
    ...
    const infoRows = [
      c.doc
        ? `<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--border-sub);font-size:12px"><span style="color:var(--text-3)">Documento</span><span>${escapeHTML(c.doc)}</span></div>`
        : '',
      c.email
        ? `<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--border-sub);font-size:12px"><span style="color:var(--text-3)">Email</span><span>${escapeHTML(c.email)}</span></div>`
        : '',
      c.telefone
        ? `<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--border-sub);font-size:12px"><span style="color:var(--text-3)">Telefone</span><span>${escapeHTML(c.telefone)}</span></div>`
        : '',
    ...
    const ordersHtml = orders.length
      ...
      <div class="drawer-order-val">${fmt(o.total)}</div>
          ...
          .join('')
      : `<div style="font-size:12px;color:var(--text-3);padding:12px 0">Nenhum pedido encontrado.</div>`;
  ...
  `
    ...
    openDrawer(
      'Erro',
      'Não foi possível abrir detalhes do cliente',
      `<div style="font-size:12px;color:var(--text-3)">Erro: ${escapeHTML(String(e?.message || e))}</div>`,
      `<button class="drawer-btn drawer-btn-ghost" onclick="closeDrawer()">Fechar</button>`,
    );
  }
}
...
function openPedidoDrawer(pedidoId) {
  try {
    ...
    const o =
      (allOrders || []).find((x) => String(x?.id ?? '') === oid) ||
      (allOrders || []).find((x) => String(x?.numero_pedido ?? '') === oid);
    if (!o) return;
    ...
    ${
      c
        ? `<div class="drawer-section"><div class="drawer-section-title">Cliente</div>
  ...
  `;
    ...
    openDrawer(
      `Pedido #${o.numero_pedido || String(o.id || '').slice(0, 8)}`,
    ...
    openDrawer(
      'Erro',
      'Não foi possível abrir detalhes do pedido',
      `<div style="font-size:12px;color:var(--text-3)">Erro: ${escapeHTML(String(e?.message || e))}</div>`,
      `<button class="drawer-btn drawer-btn-ghost" onclick="closeDrawer()">Fechar</button>`,
    );
  }
...
function renderPedidosPage() {
  const q = String((document.getElementById('ped-search') || { value: '' }).value || '')
    .toLowerCase()
    .trim();
  const sf = String((document.getElementById('ped-status-filter') || { value: '' }).value || '')
    .toLowerCase()
    .trim();
  const ch = String((document.getElementById('ped-canal-filter') || { value: '' }).value || '')
    .toLowerCase()
    .trim();
  const from = parseDateToIso(
    String((document.getElementById('ped-date-from') || { value: '' }).value || '').trim(),
  );
  const to = parseDateToIso(
    String((document.getElementById('ped-date-to') || { value: '' }).value || '').trim(),
  );
  ...
  if (kpiEl)
    kpiEl.innerHTML = [
      ...
      {
        label: 'Receita',
        val: 'R$ ' + total.toLocaleString('pt-BR', { minimumFractionDigits: 0 }),
      },
      {
        label: 'Ticket Médio',
        val: orders.length
          ? 'R$ ' + (total / orders.length).toLocaleString('pt-BR', { minimumFractionDigits: 0 })
          : '—',
      },
    ]
      .map(
        (k) =>
          `<div class="chiva-card ped-kpi-card"><div class="chiva-card-value">${escapeHTML(k.val)}</div><div class="chiva-card-label">${escapeHTML(k.label)}</div></div>`,
      )
      .join('');
...
function forceLogout(reason) {
  ...
  supaAccessToken = '';
  try {
    if (supaAuthUnsub && typeof supaAuthUnsub.unsubscribe === 'function')
      supaAuthUnsub.unsubscribe();
  } catch (_e) {}
  supaAuthUnsub = null;
...
function saveAccessUsers(users) {
  ...
  if (supaConnected && supaClient)
    sbSetConfig('crm_access_users', JSON.stringify(users)).catch((e) => {
      console.warn(
        '[config] falha ao salvar crm_access_users no Supabase:',
        e?.message || String(e),
      );
      captureError(e, { context: 'save_access_users' });
    });
}
...
async function ensureBootstrapAdminUser() {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => showBootstrapHintIfNeeded(pass), {
      once: true,
    });
  } else {
...
async function addAccessUser() {
  ...
  if (!pass || pass.length < 6) {
    if (st) {
      st.textContent = '⚠ A senha deve ter pelo menos 6 caracteres.';
      st.className = 'setup-status s-err';
    }
    return;
  }
  ...
  if (users.some((u) => normalizeAccessEmail(u.email) === email)) {
    if (st) {
      st.textContent = '⚠ Este e-mail já está cadastrado.';
      st.className = 'setup-status s-err';
    }
    return;
  }
  ...
  renderAccessUsers();
  if (st) {
    st.textContent = '✓ Usuário adicionado.';
...
function blingStateBadge(state) {
  ...
  const cls =
    s === 'ok'
      ? 'chiva-badge chiva-badge-green'
      : s === 'running'
        ? 'chiva-badge chiva-badge-amber'
        : s === 'late'
          ? 'chiva-badge chiva-badge-amber'
          : s === 'waiting'
            ? 'chiva-badge chiva-badge-amber'
            : s === 'error'
              ? 'chiva-badge chiva-badge-red'
              : 'chiva-badge';
...
async function refreshBlingAutoCard() {
  ...
  const lastTs =
    Math.max(
      isFinite(lastOrdersTs) ? lastOrdersTs : 0,
      isFinite(lastProductsTs) ? lastProductsTs : 0,
    ) || 0;
  ...
  if (msgEl) {
    ...
    if (state === 'late')
      msg = 'Sem atualização recente. Verifique se a sincronização está rodando.';
    if (state === 'error')
      msg = 'Falha ou atraso crítico. Tente sincronizar manualmente ou verifique o backend.';
...
function saveCreds() {
  ...
  localStorage.setItem('crm_csec', CSEC);
  localStorage.setItem('crm_shop', SHOP);
  localStorage.setItem('crm_shopkey', SHOPKEY);
}
...
function saveAlertDays() {
  ...
  if (supaConnected && supaClient) {
    sbSetConfig('alert_days', v).catch((e) => {
      console.warn('[config] falha ao salvar alert_days no Supabase:', e?.message || String(e));
    });
  }
}
...
function saveSupabaseConfig() {
  ...
  if (!url || !key) {
    if (st) {
      st.textContent = '⚠ Preencha URL e chave.';
      st.className = 'setup-status s-err';
    }
    return;
  }
  ...
  if (st) {
    st.textContent = '✓ Salvo! Conectando...';
    st.className = 'setup-status s-ok';
  }
  ...
  setTimeout(async () => {
    ...
    if (connected) {
      try {
        ...
        console.error('[supabase] loadSupabaseData falhou:', e?.message || e);
      }
...
function getSyncCtx() {
  return {
    ...
    setCarrinhosAbandonados: (next) => {
      carrinhosAbandonados = Array.isArray(next) ? next : [];
    },
...
function normalizeYampiOrder(o) {
  ...
  next._source = 'yampi';
  next.numero =
    next.numero || next.numero_pedido || next.order_number || next.number || next.name || next.id;
  ...
  const statusRaw =
    next.status || next.status_atual || next.financial_status || next.payment_status || '';
  next.situacao = next.situacao || { nome: String(statusRaw || '').toLowerCase() };

  const cliente = next.cliente || next.customer || next.buyer || {};
  const endereco =
    cliente.endereco || cliente.address || next.endereco_entrega || next.shipping_address || {};
  const nome =
    cliente.nome ||
    cliente.name ||
  ...
  next.contato = next.contato || {
    ...
    endereco: {
      ...
      logradouro:
        endereco.logradouro || endereco.address1 || endereco.endereco || endereco.street || '',
...
function normalizeCarrinhoAbandonado(raw) {
  ...
  const nome = String(
    c.cliente_nome || c.customer_name || c.nome || c.customer?.name || c.cliente?.nome || '',
  ).trim();
  const email = String(c.email || c.customer_email || c.customer?.email || c.cliente?.email || '')
    .trim()
    .toLowerCase();
  const telefone = String(
    c.telefone || c.phone || c.customer_phone || c.customer?.phone || c.cliente?.telefone || '',
  ).trim();
  ...
  const linkFinalizacao =
    String(
      c.link_finalizacao || c.checkout_url || c.recovery_url || c.recover_url || c.url || '',
    ).trim() || null;
...
function getPedidoItens(o) {
  ...
  raw.filter(Boolean).forEach((it) => {
    const base =
      it && typeof it === 'object' && it.item && typeof it.item === 'object' ? it.item : it;
    ...
    const codigo = String(
      base?.codigo || base?.sku || base?.id || base?.product_id || base?.produto_id || '',
    ).trim();
    const quantidade = Number(base?.quantidade ?? base?.quantity ?? base?.qty ?? 1) || 1;
    const valor =
      Number(base?.valor ?? base?.valor_unitario ?? base?.price ?? base?.preco ?? 0) || 0;
    const valor_total =
      base?.valor_total != null ? Number(base.valor_total) || 0 : quantidade * valor;
...
function calcularScoreRecuperacaoCarrinho(c, cliLookup) {
  ...
  const phone = rawPhone(c?.telefone || '');
  const cli =
    (email && cliLookup?.byEmail?.[email]) || (phone && cliLookup?.byPhone?.[phone]) || null;

  const valueCap = 500;
  const valueScore = clamp01(
    Math.log10(Math.max(0, Math.min(valor, valueCap)) + 1) / Math.log10(valueCap + 1),
  );
  const hasBoughtScore = cli && cli.n > 0 ? 1 : 0;
  const historyScore =
    cli && cli.status === 'vip' ? 1 : cli && cli.n >= 2 ? 0.75 : cli && cli.n === 1 ? 0.35 : 0;
  const timeScore =
    mins == null
      ? 0.4
      : mins < 60
  ...
  const score = Math.round(
    100 * (0.4 * valueScore + 0.25 * hasBoughtScore + 0.2 * historyScore + 0.15 * timeScore),
  );
...
async function recomputeCarrinhosScoresAndPersist() {
  try {
    carrinhosAbandonados =
      safeJsonParse('crm_carrinhos_abandonados', []) || carrinhosAbandonados || [];
  } catch (_e) {}
...
function buildCarrinhoWaMessage(c, ctx) {
  ...
  if (etapa === 'link') {
    return [
      ...
      link
        ? `Link para finalizar: ${link}`
        : 'Se você quiser, eu te mando o link pra finalizar rapidinho.',
  ...
  if (etapa === 'incentivo') {
    const incentivo =
      prioridade === 'alta'
        ? 'Se precisar, consigo te ajudar com um incentivo leve pra fechar hoje.'
...
async function openWhatsAppCarrinho(checkoutId) {
  ...
  const c = (carrinhosAbandonados || []).find((x) => String(x?.checkout_id || '') === cid);
  if (!c) {
    toast('⚠ Carrinho não encontrado');
    return;
  }
  const digits = rawPhone(c.telefone || '');
  if (!digits) {
    toast('⚠ Carrinho sem telefone');
    return;
  }
  ...
  const calc = calcularScoreRecuperacaoCarrinho(c, lookup);
  const etapa = sugerirEtapaParaCarrinho(c, calc.mins);
  const prioridade = prioridadePorScore(
    c.score_recuperacao == null ? calc.score : c.score_recuperacao,
  );
  const text = buildCarrinhoWaMessage(c, {
    etapa,
    prioridade,
    score: c.score_recuperacao == null ? calc.score : c.score_recuperacao,
  });
  ...
  if (customerKey) {
    try {
      ...
      await logInteraction(
        customerKey,
        'mensagem_enviada',
        'Recuperação carrinho abandonado (' + etapa.id + ')',
        {
...
function openRadarVisitouDrawer(checkoutId) {
  ...
  const calc = calcularScoreRecuperacaoCarrinho(c, lookup);
  const score =
    c.score_recuperacao == null
      ? Number(calc.score || 0) || 0
      : Number(c.score_recuperacao || 0) || 0;
  const etapa = sugerirEtapaParaCarrinho(c, calc.mins);
  const tempo = fmtTempoDesde(calc.mins);
  const nome = String(c.cliente_nome || 'Cliente').trim();
  const contato =
    [c.telefone ? fmtPhone(c.telefone) : '', c.email ? String(c.email) : '']
      .filter(Boolean)
      .join(' · ') || '—';
  const resumo = (Array.isArray(c.produtos) ? c.produtos : [])
    .slice(0, 4)
    .map((it) => String(it?.nome || it?.title || it?.descricao || it?.name || '').trim())
    .filter(Boolean)
    .join(', ');
...
async function reconcileCarrinhosRecuperados() {
  ...
  const bestByPhone = {};
  orders.forEach((o) => {
    const email = String(o?.contato?.email || '')
      .trim()
      .toLowerCase();
    ...
    if (email) {
      ...
      if (!cur || ts > cur.ts)
        bestByEmail[email] = { ts, id: String(o?.id || o?.numero || ''), data: dtRaw };
    }
    if (phone) {
      const cur2 = bestByPhone[phone];
      if (!cur2 || ts > cur2.ts)
        bestByPhone[phone] = { ts, id: String(o?.id || o?.numero || ''), data: dtRaw };
    }
  });
  ...
  const updated = carrinhosAbandonados.map((c) => {
    ...
    return {
      ...c,
      recuperado: true,
      recuperado_em: new Date(hit.ts).toISOString(),
      recuperado_pedido_id: hit.id || null,
    };
  });
  if (!changed) return;
  carrinhosAbandonados = updated;
...
async function fetchShopify(shop, key, from, to) {
  ...
  return (d.orders || []).map((o) => {
    ...
    o._canal = 'shopify';
    o.numero = o.name;
    o.data = o.created_at?.slice(0, 10);
    o.total = parseFloat(o.total_price) || 0;
    o.totalProdutos = o.total;
    o.situacao = { nome: mapShopifyStatus(o.financial_status) };
    o.contato = {
      nome: o.customer
        ? `${o.customer.first_name || ''} ${o.customer.last_name || ''}`.trim()
        : addr.name || 'Desconhecido',
      cpfCnpj: '',
      email: o.email || o.customer?.email || '',
      telefone: o.phone || o.customer?.phone || addr.phone || '',
      endereco: {
        municipio: addr.city || '',
        uf: addr.province_code?.split('-')[1] || '',
        logradouro: addr.address1 || '',
      },
    };
    o.itens = (o.line_items || []).map((it) => ({
      descricao: it.title,
      codigo: it.sku || '',
      quantidade: it.quantity,
      valor: parseFloat(it.price) || 0,
    }));
    return o;
  });
}
function mapShopifyStatus(s) {
  if (/paid|authorized/i.test(s || '')) return 'aprovado';
  if (/pending/i.test(s || '')) return 'pendente';
  if (/refunded|voided/i.test(s || '')) return 'cancelado';
  return s || 'outros';
}
async function syncShopify() {
  ...
  SHOPKEY = keyEl.value.trim();
  saveCreds();
  if (!SHOP || !SHOPKEY) {
    st.textContent = '⚠ Preencha URL e token';
    st.className = 'setup-status s-err';
    return;
  }
  if (!fromEl.value || !toEl.value) {
    st.textContent = '⚠ Preencha o período';
    st.className = 'setup-status s-err';
    return;
  }
  ...
  try {
    ...
    mergeOrders();
    populateUFs();
    upsertOrdersToSupabase(shopifyOrders).catch((e) => console.warn(e));
    renderAll();
    try {
  ...
  syncTimer = setInterval(
    async () => {
      try {
        ...
        console.error('[sync-timer] Falha ao sincronizar:', e?.message || e);
      }
    },
    6 * 60 * 60 * 1000,
  );
...
function showPage(id) {
  ...
  const titles = {
    dashboard: 'Dashboard',
    clientes: 'Clientes',
    inteligencia: 'Inteligência de Clientes',
    pedidos: 'Pedidos',
  ...
  if (id === 'comercial') {
    ...
    setTimeout(() => safeInvokeName('renderChartsCom'), 100);
  }
  if (id === 'marca') {
  ...
  if (id === 'dashboard') {
    ...
    if (kpisEl && !kpisEl.querySelector('.dash-kpi'))
      kpisEl.innerHTML =
        typeof renderDashKpiSkeletons === 'function' ? renderDashKpiSkeletons() : '';
  }
  ...
  if (id === 'clientes') {
    ...
    if (
      listEl &&
      !listEl.querySelector('.client-card') &&
      !listEl.querySelector('.client-card-skeleton')
    )
      listEl.innerHTML =
...
// Topbar search
const handleTopbarSearchDebounced = debounce(function (q) {
  handleTopbarSearch(q);
}, 300);
const renderClientesDebounced = debounce(function () {
  renderClientes();
}, 250);
const renderPedidosPageDebounced = debounce(function () {
  renderPedidosPage();
}, 250);
...
// ═══════════════════════════════════════════════════
function saveTasks() {
  localStorage.setItem('crm_tasks', JSON.stringify(allTasks));
}
  ...
  if (t) {
    ...
    if (supaConnected && supaClient && t._supaId) {
      ...
      supaClient
        ...
        .eq('id', t._supaId)
        .then(() => {})
        .catch((e) => console.warn('[tarefas] update falhou:', e?.message || e));
    }
  }
}
...
function deleteTask(id) {
  ...
  if (supaConnected && supaClient && t?._supaId) {
    supaClient
      ...
      .eq('id', t._supaId)
      .then(() => {})
      .catch((e) => console.warn('[tarefas] delete falhou:', e?.message || e));
  }
...
function saveTask(id) {
  ...
  if (supaConnected && supaClient) {
    ...
    const syncToSupabase = (sbTask) => {
      if (task._supaId) {
        ...
        supaClient
          .from('v2_tarefas')
          .insert({ ...sbTask, created_at: new Date().toISOString() })
          .select('id')
          .single()
          .then(({ data }) => {
            if (data?.id) {
  ...
  if (task.cliente) {
    ...
    const cust = cid
      ...
      : allCustomers.find((c) => (c.nome || '').toLowerCase() === q) ||
        allCustomers.find((c) => (c.nome || '').toLowerCase().includes(q));
    const custId = cust?.id || cid || '';
    if (custId) {
      if (!prev) {
        logInteraction(custId, 'tarefa_criada', `Tarefa: ${task.titulo}`.slice(0, 240), {
          task_id: String(task._supaId || task.id),
        }).catch(() => {});
      } else if (prev.status !== task.status && task.status === 'concluida') {
        logInteraction(
          custId,
          'tarefa_concluida',
          `Tarefa concluída: ${task.titulo}`.slice(0, 240),
          { task_id: String(task._supaId || task.id) },
        ).catch(() => {});
      }
    }
  ...
  if (!n) {
    list.forEach((c) => {
      c._rfv = {
        ...
        score: 3,
        segment: 'outros',
        dias_desde_ultima_compra: daysSince(c?.ultimo_pedido),
      };
    });
    return;
  }
  ...
  return [
    {
      id: 'champions',
      name: 'Champions',
      desc: 'Recência alta + frequência alta + valor alto (RFV)',
      filter: (c) => c?._rfv?.segment === 'champions',
    },
    {
      id: 'vip',
      name: 'VIP',
      desc: 'Muito valor + recência alta (RFV) / alto LTV',
      filter: (c) =>
        c?._rfv?.segment === 'vip' ||
        (Number(c?.total_gasto) || 0) >= 650 ||
        (Number(c?.total_pedidos) || 0) >= 6,
    },
    {
      id: 'fieis',
      name: 'Clientes fiéis',
      desc: 'Compram com frequência (RFV)',
      filter: (c) => c?._rfv?.segment === 'fieis',
    },
    {
      id: 'promissores',
      name: 'Promissores',
      desc: 'Compraram recentemente e têm potencial de recorrência (RFV)',
      filter: (c) => c?._rfv?.segment === 'promissores',
    },
    {
      id: 'novos_rfv',
      name: 'Novos clientes',
      desc: 'Primeira compra recente (RFV)',
      filter: (c) => c?._rfv?.segment === 'novos',
    },
    {
      id: 'dormindo',
      name: 'Dormindo',
      desc: '61–120 dias sem comprar (RFV/recência)',
      filter: (c) => c?._rfv?.segment === 'dormindo',
    },
    {
      id: 'perdidos',
      name: 'Clientes perdidos',
      desc: '120+ dias sem comprar (RFV/recência)',
      filter: (c) => c?._rfv?.segment === 'perdidos',
    },

    {
      id: 'risco',
      name: 'Em Risco',
      desc: 'Dias sem comprar > max(60, intervalo médio × 2) e ainda não perdido',
      filter: (c) => {
    ...
    },
    {
      id: 'recompra',
      name: 'Recompra',
      desc: '2+ pedidos e intervalo médio ≤ 30 dias',
      filter: (c) =>
        (Number(c?.total_pedidos) || 0) >= 2 && (Number(c?.intervalo_medio_dias) || 999) <= 30,
    },
    {
      id: 'alto_ticket',
      name: 'Alto Ticket',
      desc: 'Ticket médio (por pedido) ≥ R$100',
      filter: (c) => (Number(c?.ticket_medio) || 0) >= 100,
    },

    {
      id: 'low_carb',
      name: 'Low Carb Lovers',
      desc: 'Produto favorito contém: low',
      filter: (c) => segIncludes(String(c?.produto_favorito || ''), 'low'),
    },
    {
      id: 'cranberry',
      name: 'Cranberry Lovers',
      desc: 'Produto favorito contém: cranberry',
      filter: (c) => segIncludes(String(c?.produto_favorito || ''), 'cranberry'),
    },
    {
      id: 'tradicional',
      name: 'Tradicional Lovers',
      desc: 'Produto favorito contém: tradicional',
      filter: (c) => segIncludes(String(c?.produto_favorito || ''), 'tradicional'),
    },
    {
      id: 'site',
      name: 'Clientes do Site',
      desc: 'Canal principal: site/shopify',
      filter: (c) => segIncludesAny(c?.canal_principal, ['site', 'shopify']),
    },
    {
      id: 'shopee',
      name: 'Clientes Shopee',
      desc: 'Canal principal: shopee',
      filter: (c) => segIncludes(String(c?.canal_principal || ''), 'shopee'),
    },
    {
      id: 'amazon',
      name: 'Clientes Amazon',
      desc: 'Canal principal: amazon',
      filter: (c) => segIncludes(String(c?.canal_principal || ''), 'amazon'),
    },
    {
      id: 'mg',
      name: 'Clientes MG',
      desc: 'Localizados em Minas Gerais',
      filter: (c) => String(c.uf || '').toUpperCase() === 'MG',
    },
    {
      id: 'sp',
      name: 'Clientes SP',
      desc: 'Localizados em São Paulo',
      filter: (c) => String(c.uf || '').toUpperCase() === 'SP',
    },
  ];
}
...
function renderSegmentos() {
  ...
  statsHost.innerHTML = [
    ...
    {
      l: 'Champions',
      v: championsSeg?.count || 0,
      s: Math.round(((championsSeg?.count || 0) / (totalClis || 1)) * 100) + '% da base',
    },
    {
      l: 'Clientes VIP',
      v: vipSeg?.count || 0,
      s: Math.round(((vipSeg?.count || 0) / (totalClis || 1)) * 100) + '% da base',
    },
    {
      l: 'Em Risco',
      v: riscoSeg?.count || 0,
      s: Math.round(((riscoSeg?.count || 0) / (totalClis || 1)) * 100) + '% da base',
    },
    {
      l: 'Novos',
      v: novosSeg?.count || 0,
      s: Math.round(((novosSeg?.count || 0) / (totalClis || 1)) * 100) + '% da base',
    },
    ...
    .map(
      (s) =>
        `<div class="stat"><div class="stat-label">${s.l}</div><div class="stat-value">${s.v}</div><div class="stat-sub">${s.s}</div></div>`,
    )
    .join('');

  // Lista de Cards
  listHost.innerHTML = computedSegments
    .map(
      (s) => `
...
function renderSegmentCustomers() {
  ...
  const list = seg.customers.filter(
    (c) =>
      String(c.nome || '')
        .toLowerCase()
        .includes(q) ||
...
function renderSegmentCharts(seg) {
  // Cidades
  const cities = {};
  seg.customers.forEach((c) => {
    const k = c.cidade || 'Não inf.';
  ...
  const prods = {};
  seg.customers.forEach((c) => {
    const k = c.produto_favorito || 'Não inf.';
    prods[k] = (prods[k] || 0) + 1;
  });
...
function normSt(s) {
  const v = (s?.nome || s?.id || s || '').toString().toLowerCase();
...
function buildCli(list) {
  ...
  list.forEach((o) => {
    ...
    if (!m[k])
      m[k] = {
        id: k,
        nome: o.contato?.nome || 'Desconhecido',
        doc: o.contato?.cpfCnpj || '',
        email: o.contato?.email || '',
        telefone: o.contato?.telefone || o.contato?.celular || '',
        cidade: o.contato?.endereco?.municipio || '',
        uf: o.contato?.endereco?.uf || '',
        cep: o.contato?.endereco?.cep || '',
        orders: [],
        channels: new Set(),
        last: null,
        first: null,
    ...
    if (!m[k].telefone && (o.contato?.telefone || o.contato?.celular))
      m[k].telefone = o.contato?.telefone || o.contato?.celular;
...
function calcCliScores(c) {
  ...
  if (n >= 2) {
    const sorted = c.orders
      .map((o) => new Date(o.data))
      .filter((d) => !isNaN(d))
      .sort((a, b) => a - b);
  ...
  let recompraScore = 0;
  if (n >= 3) recompraScore += 40;
  else if (n === 2) recompraScore += 20;
  if (ds < 30) recompraScore += 30;
  else if (ds < 60) recompraScore += 15;
  else if (ds < 90) recompraScore += 5;
  ...
  if (recompraScore === 0 && n > 0 && localStorage.getItem('crm_debug_scores') === '1') {
    try {
      console.warn('[Score zero inesperado]', c.id, c.nome, 'pedidos:', n);
    } catch (_e) {}
  }

  // Risco de churn (0-100)
  let churnRisk = 0;
  if (ds > 120) churnRisk = 90;
  else if (ds > 90) churnRisk = 70;
  else if (ds > 60) churnRisk = 45;
  else if (ds > 30) churnRisk = 20;
  if (n === 1) churnRisk += 15;
  churnRisk = Math.min(churnRisk, 100);

  // Status
  const meta =
    typeof cliMetaCache !== 'undefined' && cliMetaCache && cliMetaCache[c.id]
      ? cliMetaCache[c.id]
      : cliMeta && cliMeta[c.id]
        ? cliMeta[c.id]
        : {};
...
function populateUFs() {
  ...
  ['fil-estado', 'fil-uf'].forEach((id) => {
    ...
    s.innerHTML =
      ...
      [...ufs]
        .sort()
        .map((uf) => `<option>${escapeHTML(uf)}</option>`)
        .join('');
    s.value = v;
  });
}
...
async function renderDashRevenueFromSupabase() {
  ...
  const selectedYear = yearSel
    ? Number(yearSel.value || storedYear || defaultYear) || defaultYear
    : storedYear || defaultYear;
  ...
  if (yearSel) {
    ...
    yearSel.value = years.includes(Number(current))
...
function detectTipoVenda(o) {
  const raw = String(o?.tipo_venda ?? o?.tipoVenda ?? '')
    .trim()
    .toLowerCase();
...
function renderDashNow() {
  ...
  if (dashSel) {
    const stored = String(localStorage.getItem('crm_dash_canal') || '')
      .toLowerCase()
      .trim();
  ...
  if (dashTipoSel) {
    const stored = String(localStorage.getItem('crm_dash_tipo') || '')
      .toLowerCase()
      .trim();
  ...
  const dashTipo = String(dashTipoSel?.value || '')
    .trim()
    .toLowerCase();
  ...
  const ordersTipo = dashTipo
    ? ordersAllRange.filter((o) => detectTipoVenda(o) === dashTipo)
    : ordersAllRange;
  const ordersSales = dashCh
    ? ordersTipo.filter(
        (o) =>
          normCanalKey(detectCh(o)) === dashCh &&
          clienteTemPedidoNoCanal(orderCustomerKey(o), dashCh),
      )
    : ordersTipo;
  const prevRange = fromIso && toIso ? calcPrevRange(fromIso, toIso) : null;
  const ordersPrevAll = prevRange
    ? ordersBase.filter((o) => {
        const raw = o?.data || o?.dataPedido || o?.data_pedido || o?.created_at || '';
        const s = String(raw || '').slice(0, 10);
        if (!s) return false;
    ...
    : [];
  const ordersPrevTipo = dashTipo
    ? ordersPrevAll.filter((o) => detectTipoVenda(o) === dashTipo)
    : ordersPrevAll;
  const ordersPrevSales = dashCh
    ? ordersPrevTipo.filter(
        (o) =>
          normCanalKey(detectCh(o)) === dashCh &&
          clienteTemPedidoNoCanal(orderCustomerKey(o), dashCh),
      )
  ...
  const now = new Date();
  const thisMo = ordersSales.filter((o) => {
    const d = new Date(o.data);
    return d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth();
  });
  const prevMo = ordersSales.filter((o) => {
    const d = new Date(o.data);
    const pm = now.getMonth() === 0 ? 11 : now.getMonth() - 1,
      py = now.getMonth() === 0 ? now.getFullYear() - 1 : now.getFullYear();
    return d.getFullYear() === py && d.getMonth() === pm;
  });
  const tMo = thisMo.reduce((s, o) => s + val(o), 0),
    pMo = prevMo.reduce((s, o) => s + val(o), 0);
  ...
  ordersBase.forEach((o) => {
    const k = orderCustomerKey(o);
    if (!k) return;
    const d = String(o?.data || o?.dataPedido || o?.data_pedido || o?.created_at || '').slice(
      0,
      10,
    );
    if (!d) return;
  ...
  try {
    ...
    if (prodEl && !prodEl.innerHTML)
      prodEl.innerHTML = `<div class="empty" style="padding:14px 0">Carregando…</div>`;
    const alertsEl = document.getElementById('dash-alerts-mini');
    if (alertsEl && !alertsEl.innerHTML)
      alertsEl.innerHTML = `<div class="empty" style="padding:14px 0">Carregando…</div>`;
    const vipsEl = document.getElementById('dash-vips-mini');
    if (vipsEl && !vipsEl.innerHTML)
      vipsEl.innerHTML = `<div class="empty" style="padding:14px 0">Carregando…</div>`;
  } catch (_e) {}
...
async function updateDashSecondaryFromSupabase() {
  ...
  if (!supaConnected || !supaClient) {
    ...
    const ltvMedio = list.length
      ? list.reduce((s, c) => s + (Number(c?.total_gasto || 0) || 0), 0) / list.length
      : 0;
  ...
  try {
    ...
    console.warn('[dashboard] falha ao carregar KPIs/funil:', e?.message || e);
  }
}
...
function renderDashKpiSparklines(ctx) {
  ...
  const firstByCustomer =
    ctx?.firstByCustomer && typeof ctx.firstByCustomer === 'object' ? ctx.firstByCustomer : {};
  ...
  const clis = Object.values(buildCli(Array.isArray(allOrders) ? allOrders : []))
    ...
    .sort((a, b) => {
      if (a.s.status !== b.s.status)
        return a.s.status === 'vip' ? -1 : b.s.status === 'vip' ? 1 : 0;
  ...
  try {
    ...
    el.innerHTML = list
      .map((p, i) => {
        ...
        const icon = (
          String(p.name).match(/[A-Za-zÀ-ÿ]/) ? String(p.name).trim()[0] : '★'
        ).toUpperCase();
...
function renderDashVipMini(_ordersSales) {
  ...
  const clis = Object.values(buildCli(Array.isArray(allOrders) ? allOrders : []))
    .map((c) => ({
      c,
      s: calcCliScores(c),
      tot: Array.isArray(c?.orders) ? c.orders.reduce((sum, o) => sum + val(o), 0) : 0,
    }))
...
function renderDashGeoMini(ordersSales) {
  ...
  (Array.isArray(ordersSales) ? ordersSales : []).forEach((o) => {
    const city = cleanText(
      o?.contato?.endereco?.municipio ||
        o?.contato?.endereco?.cidade ||
        o?.endereco?.municipio ||
        o?.cidade ||
        '',
    );
    const uf = normalizeUF(
      o?.contato?.endereco?.uf || o?.contato?.endereco?.estado || o?.uf || o?.estado || '',
    );
    if (city) cityAgg[city] = (cityAgg[city] || 0) + 1;
    if (uf) ufAgg[uf] = (ufAgg[uf] || 0) + 1;
  });
  if (listEl) {
    const list = Object.entries(cityAgg)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 6);
    if (!list.length)
      listEl.innerHTML = `<div class="empty" style="padding:14px 0">Sem cidades no período.</div>`;
    else
...
function setDashCanvasState(canvasId, hasData, msg, showClear) {
  ...
  box.style.display = '';
  const btn = showClear
    ? `<div style="margin-top:10px"><button class="btn" onclick="(function(){var s=document.getElementById('dash-canal-filter'); if(s) s.value=''; localStorage.setItem('crm_dash_canal',''); renderDash();})()">Ver todos os dados</button></div>`
    : '';
...
function renderDashSalesByDay(orders, prevOrders) {
  ...
  const prevAligned = prev.values.length
    ? prev.values
        .slice(0, values.length)
        .concat(Array(Math.max(0, values.length - prev.values.length)).fill(null))
    : [];
...
function renderDashChannelBreakdown(input) {
  ...
  if (!base.length) {
    if (charts.vendasCanal) {
      try {
        charts.vendasCanal.destroy();
      } catch (_e) {}
      charts.vendasCanal = null;
    }
    const btn = dashCh
      ? `<div style="margin-top:10px"><button class="btn" onclick="(function(){var s=document.getElementById('dash-canal-filter'); if(s) s.value=''; localStorage.setItem('crm_dash_canal',''); renderDash();})()">Ver todos</button></div>`
      : '';
    const msg = dashCh
      ? `Sem vendas via ${escapeHTML(CH[dashCh] || dashCh)} no período`
      : 'Sem dados no período';
...
async function renderDashExtraLists(ctx) {
  ...
  const dashTipo = String(ctx?.dashTipo || '')
    .trim()
    .toLowerCase();
  ...
  if (supaConnected && supaClient) {
    ...
    let topCitiesPre = null,
      semContatoPre = null,
      rowsReatPre = null,
      rowsVipPre = null;
    if (!filterActive) {
      [topCitiesPre, semContatoPre, rowsReatPre, rowsVipPre] = await Promise.all([
        getTopCidadesView(supaClient, 10).catch((e) => {
          console.warn('[dashboard] top cidades:', e?.message || e);
          return [];
        }),
        getClientesSemContatoView(supaClient, 8).catch((e) => {
          console.warn('[dashboard] sem contato:', e?.message || e);
          return [];
        }),
        getClientesReativacaoView(supaClient, 8).catch((e) => {
          console.warn('[dashboard] reativação:', e?.message || e);
          return [];
        }),
        getClientesVipRiscoView(supaClient, 8).catch((e) => {
          console.warn('[dashboard] vip risco:', e?.message || e);
          return [];
        }),
      ]);
    }
    ...
    try {
      ...
      console.warn('[dashboard] falha ao renderizar top cidades:', e?.message || e);
    }
    ...
    try {
          ...
          const rows = (clientesIntelCache || [])
            .filter(
              (r) =>
                r &&
                r.cliente_id &&
                (!r.last_interaction_at || String(r.last_interaction_at).trim() === ''),
            )
            .slice(0, 8);
      ...
      console.warn('[dashboard] falha ao renderizar sem contato:', e?.message || e);
    }
    ...
    try {
      ...
      if (riskEl) {
        if (filterActive) {
          ...
          riskEl.innerHTML =
            (Array.isArray(rowsReatPre) ? rowsReatPre : [])
              .map((c, idx) => {
                const id = escapeJsSingleQuote(String(c?.cliente_id || c?.id || ''));
                const nome = String(c?.nome || 'Cliente');
      ...
      console.warn('[dashboard] falha ao renderizar reativação:', e?.message || e);
    }
    ...
    try {
      ...
      if (vipRiskEl) {
        if (filterActive) {
          ...
          vipRiskEl.innerHTML =
            (Array.isArray(rowsVipPre) ? rowsVipPre : [])
              .map((c, idx) => {
                const id = escapeJsSingleQuote(String(c?.cliente_id || c?.id || ''));
                const nome = String(c?.nome || 'VIP');
                const dias = c?.dias_sem_comprar == null ? '' : String(c.dias_sem_comprar) + 'd';
      ...
      console.warn('[dashboard] falha ao renderizar VIP em risco:', e?.message || e);
    }

    try {
      renderDashV2NextActions(clientesIntelCache || []);
    } catch (e) {
      console.warn('[dashboard] falha ao renderizar próximas ações:', e?.message || e);
    }
  } else {
    renderDashTopCidadesFromOrders(ordersAllRange);
    const riskEl = document.getElementById('dashv2-risk');
    if (riskEl)
      riskEl.innerHTML = `<div class="empty">Conecte o Supabase para ver esta lista.</div>`;
    const vipRiskEl = document.getElementById('dashv2-vip-risk');
    if (vipRiskEl)
      vipRiskEl.innerHTML = `<div class="empty">Conecte o Supabase para ver esta lista.</div>`;
    const semContatoEl = document.getElementById('dashv2-sem-contato');
    if (semContatoEl)
      semContatoEl.innerHTML = `<div class="empty">Conecte o Supabase para ver esta lista.</div>`;
    const nextEl = document.getElementById('dashv2-next-actions');
    if (nextEl)
      nextEl.innerHTML = `<div class="empty">Conecte o Supabase para ver ações sugeridas.</div>`;
  }
}
...
function openDashActionsModal() {
  ...
  const renderRow = (r, idx) => {
    const id = escapeJsSingleQuote(
      String(r?.cliente_id || r?.cliente_uuid || r?.id || r?.cliente || ''),
    );
    const nome = String(r?.nome || r?.cliente_nome || 'Cliente');
    const act =
      String(r?.next_best_action || r?.acao_recomendada || r?.acao || '').trim() || 'Ação sugerida';
    const dias =
      r?.dias_desde_ultima_compra != null ? String(r.dias_desde_ultima_compra) + 'd' : '';
...
function getDashRangeIso() {
  ...
  const toEl = document.getElementById('dash-to');
  const fromIso =
    parseDateToIso(String(fromEl?.value || '')) ||
    String(localStorage.getItem('crm_dash_from') || '');
  const toIso =
    parseDateToIso(String(toEl?.value || '')) || String(localStorage.getItem('crm_dash_to') || '');
...
async function renderDashV2() {
  ...
  if (supaConnected && supaClient) {
    ...
    const novosSeries = (Array.isArray(novosDaily) ? novosDaily : [])
      .map((r) => ({
    ...
    novosClientesPrev = (Array.isArray(novosPrev) ? novosPrev : []).reduce(
      (s, r) => s + (Number(r?.novos_clientes || 0) || 0),
      0,
    );
    ...
    if (reativacaoEl) {
      reativacaoEl.innerHTML =
        (Array.isArray(rowsReat) ? rowsReat : [])
          .map((c, idx) => {
            const id = escapeJsSingleQuote(String(c?.cliente_id || c?.id || ''));
            const nome = String(c?.nome || 'Cliente');
            const dias =
              c?.dias_desde_ultima_compra == null ? '' : String(c.dias_desde_ultima_compra) + 'd';
            const ltv = fmtBRL(c?.ltv || c?.total_gasto || 0);
            return `<div class="top-item" onclick="openClientePage('${id}')">
          <div class="top-rank">${idx + 1}</div>
          <div class="top-name">${escapeHTML(nome)} <span style="color:var(--text-3);font-weight:600">· ${escapeHTML(dias)}</span></div>
  ...
  const kpi = [
    {
      l: 'Faturamento',
      v: fmtBRL(faturamento),
      s: dashDeltaBadge(pctDelta(faturamento, faturamentoPrev)),
    },
    ...
    {
      l: 'Novos Clientes',
      v: String(novosClientes),
      s: dashDeltaBadge(pctDelta(novosClientes, novosClientesPrev)),
    },
    {
      l: 'Clientes (base)',
      v: totalClientes == null ? '—' : String(totalClientes),
      s: ltvMedio == null ? '' : `LTV médio ${fmtBRL(ltvMedio)}`,
    },
  ];

  kpisEl.innerHTML = kpi
    .map(
      (s) =>
        `<div class="stat"><div class="stat-label">${s.l}</div><div class="stat-value">${s.v}</div><div class="stat-sub">${s.s || ' '}</div></div>`,
    )
  ...
  if (canalCanvas && canalCanvas.getContext) {
    ...
    if (ctx) {
      ...
      const sorted = Object.entries(channelAgg)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 10);
...
function renderDashV2Funil(rows) {
  ...
  const mapped = list.map((r) => {
    ...
    const value =
      Number(r?.clientes ?? r?.qtd_clientes ?? r?.total_clientes ?? r?.count ?? r?.n ?? 0) || 0;
...
function renderDashTopCidadesFromOrders(orders) {
  ...
  list.forEach((o) => {
    const cidade = String(
      o?.cidade_entrega || o?.contato?.endereco?.municipio || o?.contato?.municipio || '',
    ).trim();
...
function renderDashV2TopProdutos(rows) {
  ...
  el.innerHTML = list
    ...
    .map((r, idx) => {
      ...
      const unidades =
        Number(r?.unidades_vendidas ?? r?.unidades ?? r?.qty ?? r?.quantidade ?? 0) || 0;
...
function renderDashV2SemContato(rows) {
  ...
  el.innerHTML = list
    ...
    .map((r, idx) => {
      ...
      const ltv = fmtBRL(
        Number(r?.ltv_medio ?? r?.ltv ?? r?.receita_total ?? r?.total_gasto ?? 0) || 0,
      );
      return `<div class="top-item" onclick="openClientePage('${id}')">
      <div class="top-rank">${idx + 1}</div>
...
function renderMeta(v) {
  ...
  if (!el) return;
  if (!meta) {
    el.innerHTML = `<span style="font-size:11px;color:var(--text-3)">Clique em "Editar" para definir sua meta.</span>`;
    return;
  }
  const pct = Math.min((v / meta) * 100, 100),
    warn = pct < 50;
  el.innerHTML = `
...
async function editMeta() {
  const cur = localStorage.getItem('crm_meta') || '';
  const v = prompt('Meta mensal (R$):', cur);
  if (v === null) return;
...
function renderCompare(ordersOverride) {
  ...
  if (!Array.isArray(ordersOverride)) {
    const dashCh = String(document.getElementById('dash-canal-filter')?.value || '')
      .toLowerCase()
      .trim();
    if (dashCh)
      orders = (Array.isArray(orders) ? orders : []).filter((o) => detectCh(o) === dashCh);
  }
  const flt = (ym) => {
    const [y, m] = ym.split('-');
  ...
  const oA = flt(a),
    oB = flt(b),
    vA = oA.reduce((s, o) => s + val(o), 0),
    vB = oB.reduce((s, o) => s + val(o), 0);
...
function renderTopCli(ordersOverride) {
  ...
  orders.forEach((o) => {
    ...
    if (!k) return;
    if (!m[k]) {
      m[k] = { n: o.contato?.nome || '?', t: 0, id: k, cid: '' };
    }
    if (!m[k].cid) {
...
const SEGMENTS = [
  {
    ...
    icon: '⭐',
    name: 'VIPs',
    desc: 'Compraram R$650+, excl. CNPJ. Prioridade máxima de retenção.',
    action: 'Tratamento exclusivo + programa de fidelidade',
    filter: (c) => calcCliScores(c).status === 'vip',
  },
  {
    id: 'recorrentes',
    icon: '🔄',
    name: 'Recorrentes',
    desc: '2+ pedidos. Já confiam na marca — foco em aumentar LTV.',
    action: 'Upsell e cross-sell de novos sabores',
    filter: (c) => c.orders.length >= 2 && !isCNPJ(c.doc),
  },
  {
    id: 'prox_recompra',
    icon: '🎯',
    name: 'Prontos p/ Recompra',
    desc: 'Compraram 30-60 dias atrás. Momento ideal de abordagem.',
    action: 'WhatsApp agora — maior conversão',
    filter: (c) => {
      const d = daysSince(c.last);
      return d >= 30 && d <= 60 && !isCNPJ(c.doc);
    },
  },
  {
    id: 'risco_churn',
    icon: '⚠️',
    name: 'Risco de Perda',
    desc: 'Compraram há 61-90 dias sem voltar. Janela de resgate.',
    action: 'Oferta especial urgente via WhatsApp',
    filter: (c) => {
      const d = daysSince(c.last);
      return d >= 61 && d <= 90 && !isCNPJ(c.doc);
    },
  },
  {
    id: 'inativos',
    icon: '😴',
    name: 'Inativos',
    desc: 'Mais de 90 dias sem comprar. Reativação via campanha.',
    action: 'Campanha de reativação com desconto',
    filter: (c) => {
      const d = daysSince(c.last);
      return d > 90 && !isCNPJ(c.doc);
    },
  },
  {
    id: 'novos',
    icon: '🆕',
    name: 'Novos (1 compra)',
    desc: 'Compraram apenas 1x. Converter em recorrentes é urgente.',
    action: 'Sequência de onboarding + segundo sabor',
    filter: (c) => c.orders.length === 1 && daysSince(c.last) <= 60 && !isCNPJ(c.doc),
  },
  {
    id: 'alto_potencial',
    icon: '🚀',
    name: 'Alto Potencial',
    desc: 'Score de recompra >70. Candidatos a VIP.',
    action: 'Programa de pontos e exclusividade',
    filter: (c) => calcCliScores(c).recompraScore > 70 && !isCNPJ(c.doc),
  },
  {
    id: 'cnpj',
    icon: '🏢',
    name: 'Empresas (CNPJ)',
    desc: 'Clientes B2B. Potencial de volume mas excluídos do VIP.',
    action: 'Proposta comercial B2B',
    filter: (c) => isCNPJ(c.doc),
  },
];
...
function renderInteligencia() {
  ...
  if (!allOrders.length) {
    ...
    [topEl, riskEl, newEl, recEl, actEl].forEach((el) => {
      el.innerHTML = '';
    });
    return;
  }
  ...
  const vips = clis.filter((c) => {
    const sc = calcCliScores(c);
    return sc.status === 'vip' && !sc.isCnpj;
  });
        ...
        ? top
            .map((x) => {
              ...
              return intelRow(
                x.c,
                sub,
                `<span class="pill pill-soft">${escapeHTML(x.s.status || '')}</span>`,
              );
            })
            .join('')
        : `<div class="empty">Nenhum cliente.</div>`
    }
  `;
        ...
        ? risk
            .map((x) => {
              ...
              return intelRow(
                x.c,
                sub,
                `<span class="pill" style="background:var(--red-bg);color:var(--red)">${escapeHTML(String(x.s.churnRisk))}%</span>`,
              );
            })
            .join('')
        : `<div class="empty">Sem riscos críticos.</div>`
    }
  `;
    ...
    ${
      nov.length
        ? nov
            ...
            .join('')
        : `<div class="empty">Sem novos clientes no período.</div>`
    }
  `;
        ...
        ? rec
            .map((x) => {
              ...
              return intelRow(
                x.c,
                sub,
                `<span class="pill" style="background:var(--green-bg);color:var(--green)">${escapeHTML(String(x.c.orders.length))}×</span>`,
              );
            })
            .join('')
        : `<div class="empty">Sem recorrentes ainda.</div>`
    }
  `;
            ...
            ? vipDorm
                .map((x) => {
                  ...
                  return intelRow(
                    x.c,
                    sub,
                    `<button class="opp-mini-btn" onclick="event.stopPropagation();openTaskModal(null,'${escapeJsSingleQuote(String(x.c.nome || 'Cliente'))}')">+ Tarefa</button>`,
                  );
                })
                .join('')
            : `<div style="font-size:12px;color:var(--text-3);padding:8px 0">Nenhum VIP inativo agora.</div>`
        }
      </div>
      <div class="profile-card" style="padding:12px">
            ...
            ? inactive
                .map((x) => {
                  ...
                  return intelRow(
                    x.c,
                    sub,
                    `<button class="opp-mini-btn" onclick="event.stopPropagation();openWaModal('${escapeJsSingleQuote(String(x.c.id || ''))}')">WA</button>`,
                  );
                })
                .join('')
            : `<div style="font-size:12px;color:var(--text-3);padding:8px 0">Nenhum inativo acima do limite.</div>`
        }
      </div>
    </div>
  `;
...
function observeClientesSentinel() {
  ...
  if (!clientesIntelObserver) {
    clientesIntelObserver = new IntersectionObserver(
      (entries) => {
        ...
        loadClientesInteligenciaCache(false)
          .then(() => {
            renderClientes();
          })
          .catch(() => {});
...
function saveCurrentFilter() {
  ...
  const isEmpty =
    ...
    state.ch === 'all';
  if (isEmpty) {
    toast('Nenhum filtro ativo para salvar.', 'warning');
    return;
  }
...
function clearSelection() {
  ...
  document
    .querySelectorAll('.client-card.selected')
...
function bulkExportCSV() {
  const selected = clientesIntelCache.filter((c) =>
    selectedClientes.has(String(c.cliente_id || c.id || '')),
  );
  if (!selected.length) {
  ...
  a.href = url;
  a.download = `clientes_selecionados_${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
  URL.revokeObjectURL(url);
  toast(
    `${selected.length} cliente${selected.length !== 1 ? 's' : ''} exportado${selected.length !== 1 ? 's' : ''}!`,
    'success',
  );
  clearSelection();
}
...
function bulkCopyWhatsApp() {
  ...
  const numbers = selected
    .map((c) => String(c.celular || c.telefone || '').replace(/\D/g, ''))
    .filter(Boolean);
  if (!numbers.length) {
    toast('Nenhum número encontrado nos selecionados.', 'warning');
    return;
  }
...
function bulkMarkContacted() {
  ...
  supaClient
    ...
    .then(({ error }) => {
      if (error) {
        toast('Erro ao registrar contatos.', 'error');
        return;
      }
      toast(
        `${n} cliente${n !== 1 ? 's' : ''} marcado${n !== 1 ? 's' : ''} como contatado${n !== 1 ? 's' : ''}!`,
        'success',
      );
      clearSelection();
    })
    .catch((e) => {
      captureError(e, { context: 'bulkMarkContacted' });
      toast('Erro ao registrar contatos.', 'error');
    });
}
...
function renderCliIntelCard(c, eid, idx) {
  ...
  const actionHot =
    (isVip && (dias || 0) >= 45) || churn >= 70 || String(c?.pipeline_stage || '') === 'reativacao';
  ...
  const statusBadge = (() => {
    ...
    return status
      ? `<span class="badge" style="background:rgba(148,163,184,.10);color:var(--text-2);border-color:transparent">${escapeHTML(status)}</span>`
      : '';
  })();
  ...
  return `<div class="client-card" id="${eid}" data-cid="${escapeHTML(id)}"${iStyle}>
    ...
    ${
      phone || email
        ? `<div class="contact-bar" onclick="event.stopPropagation()">
...
function openCRMOrderDrawer(orderKey) {
  const o =
    allOrders.find((x) => String(x.id || x.numero || '') === String(orderKey)) ||
    allOrders.find((x) => String(x.numero || '') === String(orderKey));
  if (!o) return;
  const st = normSt(o.situacao);
  const ch = detectCh(o);
...
function renderClientePage() {
  ...
  const enriched = (() => {
    ...
    const telefone = cleanPhoneDigits(
      c.telefone || firstOrder.contato?.telefone || firstOrder.contato?.celular || '',
    );
    const email = cleanEmail(c.email || firstOrder.contato?.email || '');
    const doc = cleanDocDigits(
      c.doc || firstOrder.contato?.cpfCnpj || firstOrder.contato?.numeroDocumento || '',
    );
  ...
  const lastInt = meta.last_interaction_at
    ? fmtDate(String(meta.last_interaction_at).slice(0, 10))
    : null;
  const subParts = [
    c.status || '',
...
async function renderClienteTimeline(customerKey) {
  ...
  try {
    ...
    const typeBucket = (t) => {
      ...
      if (
        tt === 'pedido_criado' ||
        tt === 'pagamento_confirmado' ||
        tt === 'status_pedido_atualizado'
      )
        return 'pedido';
    ...
    host.innerHTML = `<div class="timeline">${rows
      .map((r) => {
...
async function saveInteraction() {
  ...
  overlay?.remove();
  if (document.getElementById('page-cliente')?.classList.contains('active'))
    renderClienteTimeline(customerId).catch(() => {});
  ...
  const sBadge =
    {
      ...
      alerta: `<span class="badge alerta">⚠️</span>`,
      novo: `<span class="badge novo">🆕 Novo</span>`,
      cnpj: `<span class="badge cnpj">🏢 CNPJ</span>`,
    }[scores.status] || '';
  ...
  return `<div class="client-card" id="${eid}">
    ...
    <div class="client-body">
      ...
      ${c.orders
        .map((o) => {
          const ch = detectCh(o),
            st = normSt(o.situacao);
          return `<div class="order-row"><span class="order-num">#${escapeHTML(o.numero || o.id)}</span><span>${escapeHTML(fmtDate(o.data))}</span><span class="order-val" style="color:var(--green)">${fmtBRL(val(o))}</span><span><span class="sp ${ST_CLASS[st] || 's-outros'}">${escapeHTML(ST_LABEL[st] || st)}</span></span><span><span class="badge ${ch}">${escapeHTML(CH[ch] || ch)}</span></span></div>`;
        })
        .join('')}
    </div>
  </div>`;
...
let radarOppFilters = safeJsonParse('crm_radar_filters', {
  visitou: false,
  visitou_quente: false,
  visitou_frio: false,
  vip: false,
  churn: false,
  recompra: false,
...
function saveRadarOppFilters() {
  try {
    localStorage.setItem('crm_radar_filters', JSON.stringify(radarOppFilters || {}));
  } catch (_e) {}
}

function radarToggleOppFilter(key) {
  if (!radarOppFilters || typeof radarOppFilters !== 'object')
    radarOppFilters = {
      visitou: false,
      visitou_quente: false,
      visitou_frio: false,
...
function seedOppPipeline() {
  ...
  const rebuySoon = clis
    .filter(
      (x) =>
        x.s.status !== 'cnpj' &&
        x.s.recompraScore >= 70 &&
        x.s.avgInterval &&
        x.s.ds >= Math.max(7, x.s.avgInterval - 3),
    )
...
function addOpportunity() {
  ...
  const c = allCustomers.find(
    (x) => (x.nome || '').toLowerCase().includes(q) || (x.email || '').toLowerCase().includes(q),
  );
  if (!c) {
    toast('⚠ Cliente não encontrado');
    return;
...
async function renderOportunidadesFromSupabase() {
  ...
  const q = String(document.getElementById('radar-q')?.value || '')
    .trim()
    .toLowerCase();
  if (!radarOppFilters || typeof radarOppFilters !== 'object')
    radarOppFilters = {
  ...
  chipsEl.innerHTML = chips
    .map((c) => {
      const active =
        c.id === 'todos' ? !Object.values(radarOppFilters).some(Boolean) : !!radarOppFilters[c.id];
  ...
  try {
    ...
    try {
      ...
      if (!hErr && Array.isArray(hRows)) {
        hRows.forEach((r) => {
          const id = String(
            r?.cliente_id || r?.cliente_uuid || r?.id || r?.customer_id || '',
          ).trim();
          if (!id) return;
          healthMap[id] = r;
        });
    ...
    const cartByEmail = {};
    cartsOpen.forEach((c) => {
      const em = String(c.email || c.customer_email || '')
        .trim()
        .toLowerCase();
      if (!em) return;
    ...
    const items = rows
      .map((r) => {
        ...
        const email = String(localCust?.email || r.email || '')
          .trim()
          .toLowerCase();
        ...
        const statusSaude = String(
          health?.status_saude || health?.statusSaude || segment || '',
        ).trim();
        const churnRisk =
          Number(health?.risco_churn ?? health?.churn_risk ?? r.risco_churn ?? 0) || 0;
        const recompraScore =
          Number(
            health?.score_recompra ??
              health?.recompra_score ??
              r.score_recompra ??
              intel?.score_final ??
              0,
          ) || 0;
        const diasSemComprar =
          Number(
            health?.dias_sem_comprar ??
              health?.diasSemComprar ??
              health?.dias_desde_ultima_compra ??
              health?.recencia_dias ??
              r.dias_desde_ultima_compra ??
              daysSince(health?.ultimo_pedido || r.ultimo_pedido) ??
              0,
          ) || 0;
        const totalPedidos = Number(health?.total_pedidos ?? r.total_pedidos ?? 0) || 0;
        const ltv =
          Number(health?.ltv ?? health?.valor_total ?? health?.total_gasto ?? r.total_gasto ?? 0) ||
          0;
        const ticket =
          Number(
            health?.ticket_medio ?? r.ticket_medio ?? (totalPedidos > 0 ? ltv / totalPedidos : 0),
          ) || 0;
        ...
        const nextAction = String(
          health?.next_best_action ||
            ...
            r.next_best_action ||
            fallbackNextAction(segment || statusSaude) ||
            '',
        ).trim();
    ...
    const vipRiskCount = items.filter(
      (i) =>
        String(i.segment || i.statusSaude).toLowerCase() === 'vip' &&
        (i.diasSemComprar >= 45 || i.churnRisk >= 70),
    ).length;
    const cartsCount = cartsOpen.length;
    const d30Count = items.filter((i) => i.diasSemComprar >= 30).length;
    const leadsTodayCount = visitouLeads.filter(
      (x) => String(x.criado_em || '').slice(0, 10) === todayIso,
    ).length;
    ...
    const filtered = allItems.filter((i) => {
      ...
      if (
        radarOppFilters.recompra &&
        (isLead || !(i.prob >= 65 && i.diasSemComprar >= 20 && i.diasSemComprar < 60))
      )
        return false;
    ...
    const sortByScore = (a, b) =>
      b.prob - a.prob ||
      b.potencial - a.potencial ||
      b.ltv - a.ltv ||
      b.diasSemComprar - a.diasSemComprar;
    priorityHigh.sort(sortByScore);
    highValue.sort((a, b) => b.ltv - a.ltv || sortByScore(a, b));
    rebuy.sort(sortByScore);
    risk.sort(
      (a, b) =>
        b.churnRisk - a.churnRisk || b.diasSemComprar - a.diasSemComprar || sortByScore(a, b),
    );
    ...
    const renderCard = (i) => {
      if (i.kind === 'visitou') {
        ...
        const contato =
          [i.email || '', i.phone ? fmtPhone(i.phone) : ''].filter(Boolean).join(' · ') || '—';
        const kv = `
    ...
    radarOppLastModel = {
      items: allItems,
      filtered,
      groups: { visitouHot, visitouCold, priorityHigh, highValue, rebuy, risk, inactive },
...
function enableOppDragAndSpring(host) {
  ...
  host.addEventListener('pointerdown', (ev) => {
    ...
    const clearTarget = () => {
      document
        .querySelectorAll('.kanban-col.drop-target')
    ...
    const end = async (up) => {
      ...
      if (!stage) {
        const anim = clone.animate(buildSpringKeyframes(lastDx, lastDy, 0, 0), {
          duration: 520,
          easing: 'linear',
        });
      ...
      const anim = clone.animate(buildSpringKeyframes(lastDx, lastDy, toX, toY), {
        duration: 560,
        easing: 'linear',
      });
      anim.onfinish = cleanup;
...
function radarBuildMotivo(flags) {
  ...
  if (flags.risco && flags.altoValor && flags.semContato)
    return 'Cliente valioso sem contato recente';
...
function radarScoreCustomerOpportunity(item, ctx) {
  ...
  const risco =
    segLower === 'churn' || segLower === 'em risco' || segLower === 'risco' || churnRisk >= 70;
  ...
  const lastContactDays =
    radarDaysSinceIso(item?.last_contact_at) ?? radarDaysSinceIso(item?.last_interaction_at);
  ...
  const prio =
    score >= 85
      ? { id: 'alta', label: 'Prioridade alta', cls: 'high' }
      : score >= 60
        ? { id: 'media', label: 'Prioridade média', cls: 'med' }
        : { id: 'baixa', label: 'Prioridade baixa', cls: '' };
...
function renderRadarTodayActionsBlock(state) {
  ...
  const updatedAt = state.ts
    ? new Date(state.ts).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
    : '';
  host.innerHTML = `
    <div class="radar-today">
      <div class="radar-today-head">
          ...
          .map((r) => {
            ...
            const segCls =
              segLower === 'vip'
                ...
                : segLower === 'churn'
                  ? 'seg-churn'
                  : segLower === 'em risco' || segLower === 'risco'
                    ? 'seg-risco'
                    : segLower === 'novo'
                      ? 'seg-novo'
                      : 'seg-unk';
            ...
            const badgeExtra = (r.__score?.badges || []).filter(
              (b) => b !== 'VIP' && b !== segLabel,
            );
            return `
...
async function generateRadarTodayActions() {
  ...
  radarTodayActionsState = {
    loading: true,
    last: radarTodayActionsState.last,
    ts: radarTodayActionsState.ts,
  };
  ...
  if (!model || !Array.isArray(model.items)) {
    try {
      if (supaConnected && supaClient)
        model = await renderOportunidadesFromSupabase().catch(() => null);
      else {
        renderOportunidades();
        model = radarOppLastModel;
      }
    } catch (_e) {
      model = null;
    }
  ...
  const avgLtv = customers.length
    ? customers.reduce((s, i) => s + (Number(i.ltv || 0) || 0), 0) / customers.length
    : 0;
  const avgTicket = customers.length
    ? customers.reduce((s, i) => s + (Number(i.ticket || 0) || 0), 0) / customers.length
    : 0;
  ...
  try {
    document
      .getElementById('radar-today-actions')
...
async function openOppClienteResumo(customerKey) {
  ...
  openDrawer(
    ...
    'Resumo de compras',
    body,
    `<button class="drawer-btn drawer-btn-ghost" onclick="closeDrawer()">Fechar</button><button class="drawer-btn drawer-btn-primary" onclick="openClientePage('${escapeJsSingleQuote(key)}')">Abrir perfil</button>`,
  );
}
...
function renderOportunidades() {
  ...
  const items = clis.map((x) => {
    ...
    const nextAction =
      s.nextBestAction ||
      (segment === 'vip'
        ? 'Oferta VIP: lançamento/kit exclusivo'
        : segment === 'churn'
          ? 'Reativar com oferta forte + mensagem pessoal'
  ...
  rebuy.sort(sortByScore);
  risk.sort(
    (a, b) => b.churnRisk - a.churnRisk || b.diasSemComprar - a.diasSemComprar || sortByScore(a, b),
  );
  inactive.sort((a, b) => b.diasSemComprar - a.diasSemComprar || sortByScore(a, b));
...
async function saveCliStatus(id, status) {
  ...
  if (status) cliMeta[id].status = status;
  else delete cliMeta[id].status;
  ...
  if (supaConnected && supaClient) {
    ...
    if (uuid)
      supaClient
        .from('v2_clientes')
        .update({ status_manual: status || null, updated_at: new Date().toISOString() })
        .eq('id', uuid)
        .then(() => {});
  }
  toast('✓ Salvo!');
...
async function saveNote(id, inputId) {
  ...
  if (supaConnected && supaClient) {
    ...
    if (uuid)
      supaClient
        .from('v2_clientes')
        .update({ notas: v, updated_at: new Date().toISOString() })
        .eq('id', uuid)
        .then(() => {});
  }
...
function openWa(phone, name, perfil) {
  ...
  const tplsPerfil = {
    vip: [
      'Oi {nome}! 🌟 Você é um dos nossos clientes mais especiais! Preparamos uma condição exclusiva VIP pra você. Posso te contar?',
      'Olá {nome}! Como VIP Chiva Fit, você tem acesso antecipado às nossas novidades 🚀 Quer saber o que vem por aí?',
      'Oi {nome}! Obrigada por confiar tanto na Chiva Fit ⭐ Você tem desconto especial esperando por você!',
    ],
    inativo: [
      'Oi {nome}! 😊 Sentimos sua falta na Chiva Fit! Que tal voltar com condições especiais pra você? 💪',
      'Olá {nome}! Faz um tempinho que não nos vemos... Temos novos sabores incríveis e uma oferta exclusiva pra te trazer de volta 🥤',
      'Oi {nome}! A Chiva Fit tem uma surpresa especial pra quem volta! Bora conversar? 🎁',
    ],
    recompra: [
      'Oi {nome}! Tá na hora de reabastecer? 🔥 Seu produto favorito pode estar acabando! Posso ajudar?',
      'Olá {nome}! Com base no seu histórico, você deve estar quase no fim do seu shake 💪 Quer garantir o próximo com desconto?',
      'Oi {nome}! Não deixa faltar sua suplementação! Temos combos especiais essa semana 🏋️',
    ],
    novo: [
      'Oi {nome}! Seja bem-vinda à família Chiva Fit! 🎉 Ficou alguma dúvida sobre seu pedido?',
      'Olá {nome}! Como foi sua primeira experiência com a Chiva Fit? Adoraria saber sua opinião 😊',
      'Oi {nome}! Que tal conhecer todos os sabores da Chiva Fit? Temos uma sequência incrível pra te sugerir 🥤',
    ],
    default: WA_TPLS,
  };
  const tpls = tplsPerfil[perfil] || tplsPerfil.default;
  const tplsFormatados = tpls.map((t) => t.replace(/\{nome\}/g, name.split(' ')[0]));
  document.getElementById('wa-templates').innerHTML = tplsFormatados
    .map(
      (t, i) =>
        `<div class="wa-tpl" id="wt${i}" onclick="selectTpl(${i})"><div class="wa-tpl-label">${escapeHTML(['💎 VIP', '🔄 Reengajamento', '⚡ Urgente'][i] || 'Template ' + (i + 1))}</div><div>${escapeHTML(t).replace(/\n/g, '<br>')}</div></div>`,
    )
    .join('');
...
function selectTpl(i) {
  document.querySelectorAll('.wa-tpl').forEach((el, j) => el.classList.toggle('selected', j === i));
  const el = document.querySelectorAll('.wa-tpl')[i];
  if (el)
    document.getElementById('wa-custom').value = el.innerText
      .split('\n')
      .slice(1)
      .join('\n')
      .trim();
...
function sendWa() {
  ...
  window.open(url, '_blank');
  if (waCustomerId) {
    logInteraction(waCustomerId, 'mensagem_enviada', msg.slice(0, 240), {
      channel: 'whatsapp',
    }).catch(() => {});
  }
  closeWa();
...
function saveTemplates() {
  WA_TPLS = [1, 2, 3].map((i) => document.getElementById('tpl' + i)?.value || '').filter(Boolean);
  localStorage.setItem('crm_wa_tpls', JSON.stringify(WA_TPLS));
  document.getElementById('tpl-status').textContent = '✓ Salvos!';
  document.getElementById('tpl-status').className = 'setup-status s-ok';
}
...
function openProdutoDrawer(prodKey) {
  ...
  const estoqueTxt =
    p?.estoque == null ? '—' : String(Number(p.estoque || 0).toLocaleString('pt-BR'));
...
function renderProdutos(_deferred) {
  ...
  document.getElementById('prod-label').textContent =
    `${prods.length} produto${prods.length !== 1 ? 's/sabores' : ''}`;
  ...
  const rankingsHtml = `
    ...
    <div style="background:var(--card);border-radius:14px;padding:16px;border:1px solid var(--border);box-shadow:0 2px 6px rgba(0,0,0,0.04)">
      ...
      ${prods
        .sort((a, b) => b.peds.size - a.peds.size)
        .slice(0, 10)
        .map(
          (p, i) => `
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
      ...
      <tbody>
        ${prods
          .map((p) => {
            ...
            if (p.total > avgTotal * 1.5 || p.qty > avgQty * 1.5)
              status = '<span class="chiva-badge chiva-badge-green">🟢 LÍDER</span>';
            else if (p.total < avgTotal * 0.5)
              status = '<span class="chiva-badge chiva-badge-red">🔴 BAIXO</span>';

            const canaisHtml = Object.entries(p.canais)
              .filter(([, q]) => q > 0)
              .map(
                ([c, q]) =>
                  `<span style="font-size:9px;background:var(--border);padding:2px 5px;border-radius:4px;margin-right:3px" title="${CH[c] || c}">${(CH[c] || c).slice(0, 3).toUpperCase()}: ${q}</span>`,
              )
              .join('');
...
async function renderCidades() {
  ...
  const salesMap = {};
  const stateMap = {};
  geoEstados.forEach((e) => {
    stateMap[e.sigla] = { ...e, total: 0, clis: new Set(), peds: 0 };
  });
  ...
  const topEstado = Object.values(stateMap).sort((a, b) => b.total - a.total)[0] || {
    sigla: '—',
    total: 0,
  };
  ...
  const top20 = Object.values(salesMap)
    .sort((a, b) => b.total - a.total)
    .slice(0, 20);
  document.getElementById('geo-top-cities').innerHTML =
    top20
      .map(
        (c, i) => `
...
function renderBrazilMap(stateMap, options) {
  ...
  const canal = String(options?.canal || '')
    .toLowerCase()
    .trim();
  ...
  ensureBrazilMapSvg(container)
    .then((svg) => {
      ...
      stateEls.forEach((el) => {
        const uf = String(el.getAttribute('id') || '')
          .trim()
          .toUpperCase();
        ...
        const clisCount =
          s?.clis && typeof s.clis.size === 'number' ? s.clis.size : Number(s?.clisCount || 0) || 0;
      ...
      if (!brazilMapBound) {
        ...
        const show = (target, clientX, clientY) => {
          ...
          const uf = String(target?.getAttribute('id') || '')
            .trim()
            .toUpperCase();
...
function exportClientesCSV() {
  const source = clientesIntelCache.length
    ? clientesIntelCache
    : Object.values(buildCli(allOrders));
  if (!source.length) {
    toast('Nenhum cliente para exportar', 'warning');
    return;
  }
  const header = [
    'Nome',
...
function exportPedidosCSV() {
  ...
  if (!orders.length) {
    toast('Nenhum pedido para exportar', 'warning');
    return;
  }
  const header = [
    'Número',
...
function handleSyncError(context, error, meta) {
  ...
  if (/Failed to fetch|NetworkError|fetch failed|timeout|ECONN/i.test(msg))
...
function ensurePendingOpsPump() {
  ...
  pendingOpsTimer = setInterval(() => {
    flushPendingOps().catch((e) => {
      console.warn(
        '[pendingOps] falha ao sincronizar operações pendentes:',
        e?.message || String(e),
      );
    });
  }, 30_000);
...
async function loadClienteMetaCache() {
  ...
  try {
    const { data } = await supaClient
      .from('v2_clientes')
      .select(
        'id,doc,status_manual,notas,pipeline_stage,last_interaction_at,last_interaction_type,last_contact_at,responsible_user',
      )
      .limit(5000);
    const next = {};
    (data || []).forEach((c) => {
...
function maskEmailForLog(v) {
  ...
  const domMasked = dom
    ? '***' + (dom.includes('.') ? dom.slice(dom.lastIndexOf('.')) : '')
    : '***';
...
function sanitizeForSupabaseLog(obj) {
  ...
  if (!Array.isArray(next)) {
    ...
    if ('telefone' in next)
      next.telefone = maskDigitsForLog(next.telefone) || String(next.telefone || '');
    if ('email' in next) next.email = maskEmailForLog(next.email) || String(next.email || '');
    if ('customer_email' in next)
      next.customer_email =
        maskEmailForLog(next.customer_email) || String(next.customer_email || '');
    if ('customer_phone' in next)
      next.customer_phone =
        maskDigitsForLog(next.customer_phone) || String(next.customer_phone || '');
  }
  return next;
}
...
async function loadInsumosFromSupabase() {
  ...
  try {
    const { data, error } = await supaClient
      .from('insumos')
      .select(
        'id,nome,unidade,estoque_atual,estoque_minimo,custo_unitario,fornecedor,lead_time_dias,updated_at',
      )
      .limit(5000);
...
async function loadReceitasProdutosFromSupabase() {
  ...
  try {
    ...
    const prods = Array.from(
      new Set((data || []).map((r) => String(r.produto_id || '').trim()).filter(Boolean)),
...
async function syncReceitasToSupabase(list) {
  ...
  try {
    ...
    handleSyncError('syncReceitasToSupabase', e, {
      table: 'receitas_produtos',
      rows,
      onConflict: 'id',
    });
    return false;
...
async function loadOrdensProducaoFromSupabase() {
  ...
  try {
    const { data, error } = await supaClient
      ...
      .select(
        'id,lote,produto_id,quantidade_planejada,quantidade_produzida,data_producao,status,observacoes,created_at',
      )
      .limit(5000);
    if (error || !Array.isArray(data)) return;
...
async function logMovimentoEstoque(mov) {
  ...
  try {
    ...
    if (error) {
      ...
      const { error: e2 } = await supaClient
        .from('movimentos_estoque')
        .upsert(fallback, { onConflict: 'ordem_id,insumo_id,tipo' });
      if (e2) throw e2;
    }
    setSyncStatus('ok');
  } catch (e) {
    if (payloadWithColumns)
      handleSyncError('logMovimentoEstoque', e, {
        table: 'movimentos_estoque',
        rows: [payloadWithColumns],
        onConflict: 'ordem_id,insumo_id,tipo',
      });
  }
}
...
async function initSupabase() {
  ...
  try {
    ...
    if (!supaSession?.access_token) {
      if (st) {
        st.textContent = '⚠ Faça login novamente (Supabase Auth).';
        st.className = 'setup-status s-err';
      }
      supaConnected = false;
      setSyncDot(false);
      ...
      allTasks = allTasks.map((local) => {
        if (local._supaId && sbById[local._supaId]) {
          ...
          return {
            ...
            titulo: srv.titulo,
            desc: srv.descricao || '',
            prioridade: srv.prioridade || local.prioridade,
            status: srv.status === 'aberta' ? 'pendente' : srv.status,
            data: srv.vencimento || '',
            customer_id: String(srv.cliente_id || local.customer_id || ''),
          };
        }
        return local;
      });
...
function updateClientesIntelFiltersOptions() {
  try {
    ...
    if (segSel) {
      ...
      segSel.innerHTML =
        `<option value="">Segmento CRM</option>` +
        segs.map((s) => `<option value="${escapeHTML(s)}">${escapeHTML(s)}</option>`).join('');
...
async function loadClientesInteligenciaCache(forceReset = false) {
  ...
  if (
    !forceReset &&
    !clientesIntelCursor &&
    clientesIntelLoadedAt &&
    Date.now() - clientesIntelLoadedAt < 3 * 60 * 1000 &&
    clientesIntelCache.length
  )
    return;
  ...
  try {
    ...
    const pageSize = 500;
    const res = await getClientesInteligenciaView(supaClient, {
      pageSize,
      cursor: clientesIntelCursor,
    });
    ...
    if (!hasMore && clientesIntelCache.length > 0 && clientesIntelCache.length % pageSize === 0) {
      console.warn(
        `[Clientes] Total carregado (${clientesIntelCache.length}) é múltiplo exato do pageSize — pode haver mais registros.`,
      );
    }
  } catch (_e) {
...
async function auditSupabaseSchema() {
  ...
  if (outEl) {
    outEl.textContent = 'Auditando...';
    outEl.className = 'setup-status';
  }
  try {
    const connected = await initSupabase();
    if (!connected) {
      if (outEl) {
        outEl.textContent = '⚠ Conecte o Supabase primeiro.';
    ...
    const checks = [
      {
        table: 'insumos',
        cols: 'id,nome,unidade,estoque_atual,estoque_minimo,custo_unitario,fornecedor,lead_time_dias,updated_at',
      },
      {
        table: 'receitas_produtos',
        cols: 'id,produto_id,insumo_id,quantidade_por_unidade,unidade,updated_at',
      },
      {
        table: 'ordens_producao',
        cols: 'id,lote,produto_id,quantidade_planejada,quantidade_produzida,data_producao,status,observacoes,created_at',
      },
      {
        table: 'movimentos_estoque',
        cols: 'id,insumo_id,ordem_id,tipo,quantidade,unidade,created_at,metadata',
      },
      {
        table: 'interactions',
        cols: 'id,customer_id,type,description,created_at,user_responsible,source,metadata',
      },
      { table: 'v2_clientes', cols: 'id,doc,nome,email,telefone,cidade,uf' },
      {
        table: 'v2_pedidos',
        cols: 'id,numero_pedido,bling_id,cliente_id,canal_id,data_pedido,total,status,source,created_at',
      },
      { table: 'v2_produtos', cols: 'id,codigo,nome,estoque,preco,situacao,origem,updated_at' },
      {
        table: 'customer_intelligence',
        cols: 'cliente_id,score_final,next_best_action,updated_at',
      },
      {
        table: 'carrinhos_abandonados',
        cols: 'checkout_id,cliente_nome,telefone,email,valor,produtos,criado_em,recuperado,recuperado_em,recuperado_pedido_id,score_recuperacao,link_finalizacao,last_etapa_enviada,last_mensagem_at',
      },
...
function normalizeOrderForCRM(o, sourceHint) {
  ...
  const municipio =
    endereco.municipio || endereco.cidade || endereco.city || endereco.localidade || '';
  const uf = normalizeUF(
    endereco.uf ||
      endereco.estado ||
      endereco.state ||
      endereco.province ||
      endereco.province_code ||
      '',
  );
...
async function loadOrdersFromSupabaseForCRM() {
  ...
  let itemsByPedidoId = {};
    ...
    const extractYampiCustomer = (raw) => {
      ...
      const shipping =
        obj.shipping_address || obj.shippingAddress || obj.shipping || obj.address || {};
      const billing = obj.billing_address || obj.billingAddress || {};
      const addr =
        shipping && typeof shipping === 'object' && Object.keys(shipping).length
          ? shipping
          : billing;
      const name =
        customer.name ||
      ...
      const state =
        addr.state || addr.uf || addr.estado || addr.province || addr.province_code || '';
    ...
    (pedRows || []).forEach((p) => {
      ...
      const canalSlugRaw = String(p.canal_slug || p.canalSlug || p.canal || p.channel || '')
        .trim()
        .toLowerCase();
      ...
      const o = {
        ...
        itens:
          pid && Array.isArray(itemsByPedidoId[pid]) && itemsByPedidoId[pid].length
            ? itemsByPedidoId[pid]
    ...
    (yampiRows || []).forEach((y) => {
      ...
      const exists = nextYampi.some(
        (ex) => ex.id === normalized.id || ex.numero === normalized.numero,
      );
    ...
    if (persistBack && (nextYampi.length || nextBling.length)) {
      const shouldBackfill =
        !Array.isArray(pedRows) ||
        pedRows.length === 0 ||
        !Array.isArray(cliRows) ||
        cliRows.length === 0;
      if (shouldBackfill) {
...
async function upsertOrdersToSupabase(orders) {
  ...
  try {
      ...
      (Array.isArray(rows) ? rows : []).forEach((r) => {
        ...
        if (bad) {
          ...
          try {
            console.warn(
              label,
              'registro ignorado (campo obrigatório ausente)',
    ...
    const { ok: validCliRows } = splitValidRows(
      filteredCliRows,
      ['doc', 'nome'],
      '[Upsert v2_clientes]',
    );
      ...
      if (error) {
        ...
        try {
          console.error(
            '[Upsert v2_clientes]',
            error,
            sanitizeForSupabaseLog(payload.slice(0, 10)),
          );
        } catch (_e) {}
        ...
        for (const row of payload) {
          try {
            ...
            if (rowErr) {
              ...
              try {
                console.warn(
                  '[Upsert v2_clientes] registro ignorado (erro no upsert)',
    ...
    const docToUuid = {};
    const docsToResolve = Array.from(
      new Set(validCliRows.map((r) => String(r?.doc || '').trim()).filter(Boolean)),
    );
    ...
    const pedRows = orders
      .map((o) => {
        const docDigits = String(o.contato?.cpfCnpj || o.contato?.numeroDocumento || '').replace(
          /\D/g,
          '',
        );
        const doc =
    ...
    try {
      ...
      list.forEach((o) => {
        ...
        itens.forEach((it) => {
          ...
          const nome = String(
            it?.descricao || it?.produto_nome || it?.title || it?.nome || it?.name || '',
          ).trim();
    ...
    logSupabaseUpsertError('upsert orders error', e, {
      orders: Array.isArray(orders) ? orders.slice(0, 2) : null,
    });
    try {
      console.error('[Upsert orders]', e, {
        orders: Array.isArray(orders) ? orders.slice(0, 2) : null,
      });
    } catch (_e) {}
    toast('⚠️ Erro ao salvar dados. Ver console F12.');
...
async function runClienteDebug(customerKey) {
  ...
  if (clienteId) {
    try {
      ...
      if (first?.cliente_id != null)
        result.match_clienteId = String(first.cliente_id) === String(clienteId);
    } catch (e) {
    ...
    try {
      ...
      if (ok && result.pedidos.length) {
        const ids = Array.from(new Set(result.pedidos.map((p) => p?.id).filter(Boolean))).slice(
          0,
          500,
        );
...
function setupRealtimeSync() {
  ...
  try {
    ...
    supaClient
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'v2_clientes' }, async () => {
        ...
        if (document.getElementById('page-oportunidades')?.classList.contains('active'))
          renderOportunidades();
        if (
          document.getElementById('page-cliente')?.classList.contains('active') &&
          currentClienteId
        )
          renderClienteTimeline(currentClienteId).catch(() => {});
        if (document.getElementById('page-inteligencia')?.classList.contains('active'))
          renderInteligencia();
        toast('🔄 Dados sincronizados!');
      })
      .subscribe();
    supaClient
      .channel('interactions-rt')
    ...
    supaClient
      ...
      .subscribe();
    supaClient
      .channel('receitas-produtos-rt')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'receitas_produtos' },
        async () => {
          await loadReceitasProdutosFromSupabase();
        },
      )
    ...
    supaClient
      ...
      .subscribe();
    supaClient
      .channel('carrinhos-abandonados-rt')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'carrinhos_abandonados' },
        async () => {
          await loadCarrinhosAbandonadosFromSupabase();
        },
      )
    ...
    supaClient
      ...
      .on(
        ...
        async (payload) => {
          if (payload.new?.chave === 'meta_mensal') {
            localStorage.setItem('crm_meta', payload.new.valor_texto || '0');
            renderDash();
          }
          if (payload.new?.chave === 'alert_days') {
            localStorage.setItem('crm_alertdays', payload.new.valor_texto || '60');
            renderAlertas();
          }
          if (payload.new?.chave === 'crm_access_users') {
            localStorage.setItem('crm_access_users', payload.new.valor_texto || '[]');
            renderAccessUsers();
          }
        },
      )
...
let allCampanhas = safeJsonParse('crm_campanhas', null) || [
  {
    id: 1,
    nome: 'Semana do Chocolate',
    canal: 'shopee',
    tipo: 'desconto',
    inicio: '2025-03-15',
    fim: '2025-03-21',
    oferta: '25% off linha chocolate',
    budget: 500,
    meta: 8000,
    status: 'encerrada',
    desc: '',
  },
  {
    ...
    inicio: '2025-03-29',
    fim: '2025-03-30',
    oferta: 'Frete grátis acima de R$89',
    budget: 200,
    meta: 5000,
    status: 'planejada',
    desc: '',
  ...
  {
    id: 3,
    nome: 'Lançamento Sabor Açaí',
    canal: 'todos',
    tipo: 'lancamento',
    inicio: '2025-04-07',
    fim: '2025-04-14',
    oferta: 'R$10 off no primeiro pedido',
    budget: 1200,
    meta: 15000,
    status: 'planejada',
    desc: '',
...
function saveCampanhas() {
  localStorage.setItem('crm_campanhas', JSON.stringify(allCampanhas));
}
...
function normalizeCanal(origem) {
  ...
  if (
    s === 'ml' ||
    s === 'mercado_livre' ||
    /\bmercado\s*livre\b|\bmercadolivre\b|\bmeli\b|\bmlb\b/.test(s)
  )
    return { slug: 'mercado_livre', nome: 'Mercado Livre', emoji: '🟡', color: 'var(--ml)' };
  if (s === 'shopee' || /\bshopee\b/.test(s))
    return { slug: 'shopee', nome: 'Shopee', emoji: '🟠', color: 'var(--shopee)' };
  if (s === 'amazon' || /\bamazon\b/.test(s))
    return { slug: 'amazon', nome: 'Amazon', emoji: '🟦', color: 'var(--amazon)' };
  if (s === 'shopify' || /\bshopify\b|\bloja\s*online\b|\becommerce\b|\bsite\b/.test(s))
    return {
      slug: 'shopify',
  ...
  if (s === 'yampi' || /\byampi\b/.test(s))
    return { slug: 'yampi', nome: 'Yampi / Checkout', emoji: '🟣', color: 'var(--yampi)' };
  if (s === 'b2b' || s === 'cnpj' || /\bb2b\b|\bcnpj\b|\batacado\b/.test(s))
    return { slug: 'b2b', nome: 'B2B / Atacado', emoji: '🟤', color: 'var(--amber)' };
...
function canalFromOrder(o) {
  ...
  const sourceHint = String(o?._source || o?.source || '')
    .toLowerCase()
    .trim();
...
function getComPedidosBase() {
  ...
  const orders = baseOrders
    ...
    .sort(
      (a, b) =>
        new Date(b.data || b.data_pedido || b.created_at || 0) -
        new Date(a.data || a.data_pedido || a.created_at || 0),
    );
  return orders.map((o) => {
    const numRaw = o?.numero_pedido || o?.numero || o?.name || o?.order_number || o?.id || '';
    const num = numRaw
      ? String(numRaw).startsWith('#')
        ? String(numRaw)
        : '#' + String(numRaw)
      : '#—';
    ...
    return {
      id: String(o?.id || numRaw || cryptoRandomId()),
      num,
      cliente,
      canal: canalInfo.slug,
      canal_nome: canalInfo.nome,
      canal_color: canalInfo.color,
      canal_emoji: canalInfo.emoji,
      produto,
      valor,
      status,
      data,
...
function renderComKpis() {
  var el = document.getElementById('com-kpis');
  if (!el) return;
  var pedidos = getComPedidosBase();
  var receita = pedidos.reduce(function (s, p) {
    return s + p.valor;
  }, 0);
  var ativas = allCampanhas.filter(function (c) {
  ...
  el.innerHTML =
    kpiCard('Pedidos', pedidos.length, 'no período', 'var(--text)') +
    kpiCard(
      'Receita',
    ...
    kpiCard('Campanhas Ativas', ativas, 'rodando', 'var(--indigo-hi)') +
    kpiCard('Canais', canais, 'ativos', 'var(--amber)');
}

function renderComPedidos() {
  var el = document.getElementById('com-pedidos-list');
  if (!el) return;
  ...
  if (!list.length) {
    el.innerHTML = '<div class="empty">Nenhum pedido encontrado</div>';
    return;
  }
  var stL = {
    novo: '🆕 Novo',
  ...
  var stC = {
    novo: 'var(--blue)',
    separando: 'var(--amber)',
    enviado: 'var(--indigo-hi)',
    entregue: 'var(--green)',
    cancelado: 'var(--red)',
  };
  var stB = {
    novo: 'var(--blue-bg)',
    separando: 'var(--amber-bg)',
    enviado: 'var(--indigo-bg)',
    entregue: 'var(--green-bg)',
  ...
  var header =
    '<div class="table-head table-head-com"><span>Pedido</span><span>Cliente / Produto</span><span>Canal</span><span class="ta-r">Valor</span><span class="ta-r">Status</span></div>';
  el.innerHTML =
    header +
    list
      .map(function (p) {
        return (
          '<div class="table-row table-row-com">' +
          '<div><div class="mono-link">' +
          escapeHTML(p.num) +
          '</div><div class="muted-xs">' +
          escapeHTML(p.data) +
          '</div></div>' +
          ...
          '</div></div>' +
          '<div><span class="pill pill-soft" style="color:' +
          (cC[p.canal] || p.canal_color || 'var(--text-2)') +
          '">' +
          escapeHTML(p.canal_nome || String(p.canal || '').toUpperCase()) +
          '</span></div>' +
          '<div class="ta-r mono-strong">R$' +
          p.valor.toFixed(2) +
          '</div>' +
          '<div class="ta-r"><span class="pill" style="background:' +
          stB[p.status] +
          ';color:' +
          stC[p.status] +
          '">' +
          escapeHTML(stL[p.status] || p.status) +
          '</span></div>' +
          '</div>'
        );
      })
...
function renderCanaisGrid() {
  var el = document.getElementById('canais-grid');
  if (!el) return;
  var por = {};
  getComPedidosBase().forEach(function (p) {
    ...
    if (!por[key])
      por[key] = {
        ...
        receita: 0,
        info: {
          nome: p.canal_nome || key,
          emoji: p.canal_emoji || '🔹',
          color: p.canal_color || 'var(--text-2)',
        },
      };
    por[key].qtd++;
    por[key].receita += p.valor;
  });
  var sorted = Object.entries(por).sort(function (a, b) {
    return (b[1].receita || 0) - (a[1].receita || 0);
  });
  el.innerHTML = sorted
    .map(function (e) {
      var dados = e[1],
        info = dados.info || { nome: e[0], emoji: '🔹', color: 'var(--text-2)' };
      var ticket = dados.qtd ? dados.receita / dados.qtd : 0;
      return (
        '<div class="canal-card"><div style="display:flex;align-items:center;gap:12px;margin-bottom:12px"><div style="font-size:24px">' +
        info.emoji +
        '</div><div><div style="font-size:13px;font-weight:800">' +
        escapeHTML(info.nome) +
        '</div><div style="font-size:10px;color:var(--text-3)">' +
        dados.qtd +
        ' pedidos</div></div></div><div style="font-size:22px;font-weight:800;font-family:var(--mono);color:' +
        info.color +
        ';margin-bottom:8px">R$' +
        dados.receita.toLocaleString('pt-BR', { minimumFractionDigits: 0 }) +
        '</div><div style="font-size:10px;color:var(--text-3)">Ticket médio: <b style="color:var(--text)">R$' +
        ticket.toFixed(2) +
        '</b></div></div>'
      );
    })
...
function renderCampanhas() {
  var el = document.getElementById('campanhas-list');
  if (!el) return;
  if (!allCampanhas.length) {
    el.innerHTML = '<div class="empty">Nenhuma campanha cadastrada</div>';
    return;
  }
  var stC = {
    ativa: 'cs-ativa',
    planejada: 'cs-planejada',
    pausada: 'cs-pausada',
    encerrada: 'cs-encerrada',
  };
  var stL = {
    ativa: '🟢 Ativa',
    planejada: '📋 Planejada',
  ...
  el.innerHTML = []
    ...
    .map(function (c) {
      return (
        '<div class="campanha-card"><div><div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><span style="font-size:16px">' +
        (tE[c.tipo] || '📣') +
        '</span><div style="font-size:13px;font-weight:800">' +
        escapeHTML(c.nome) +
        '</div></div><div style="font-size:10px;color:var(--text-3);margin-bottom:8px">' +
        escapeHTML(fmtDate(c.inicio)) +
        ' → ' +
        escapeHTML(fmtDate(c.fim)) +
        ' · ' +
        escapeHTML(String(c.canal || '').toUpperCase()) +
        ' · ' +
        escapeHTML(c.oferta) +
        '</div><div style="display:flex;gap:16px"><span style="font-size:10px;color:var(--text-3)">Budget: <b style="color:var(--text)">R$' +
        c.budget.toLocaleString('pt-BR') +
        '</b></span><span style="font-size:10px;color:var(--text-3)">Meta: <b style="color:var(--green)">R$' +
        c.meta.toLocaleString('pt-BR') +
        '</b></span></div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px"><span class="camp-status ' +
        stC[c.status] +
        '">' +
        escapeHTML(stL[c.status] || c.status) +
        '</span><button onclick="abrirModalCampanha(' +
        c.id +
        ')" style="background:none;border:1px solid var(--border);border-radius:var(--r-md);padding:4px 10px;color:var(--text-2);font-size:10px;font-weight:700;cursor:pointer;font-family:var(--font)">Editar</button></div></div>'
      );
    })
    .join('');
}
...
function renderCarrinhosKpis(list) {
  ...
  const quentes = abertos.filter((c) => {
    const m = c.tempo_min;
    return m != null && m < 120;
  });
  ...
  el.innerHTML = [
    {
      icon: '💰',
      value: fmtBRL(valorRisco),
      label: 'Valor em risco',
      sub: `${abertos.length} carrinho${abertos.length !== 1 ? 's' : ''} aberto${abertos.length !== 1 ? 's' : ''}`,
      cls: '',
      i: 0,
    },
    {
    ...
    },
    {
      ...
      sub: fmtBRL(valorRec) + ' recuperado',
      cls: ' green',
      i: 2,
    },
    {
      icon: '🎯',
      value: fmtBRL(ticketMed),
      label: 'Ticket médio',
      sub: 'Média dos carrinhos abertos',
      cls: '',
      i: 3,
    },
  ]
...
function pipelineBadge(c) {
  ...
  if (etapa && etapa !== 'aguardar')
    return `<span class="pipe-badge pipe-contatado">💬 Contatado</span>`;
...
function buildRespSelect(safeId, currentResp) {
  ...
  if (
    ...
    !users.find(function (u) {
      return String(u.email || '') === cur;
    })
  ) {
    opts += '<option value="' + escapeHTML(cur) + '" selected>' + escapeHTML(cur) + '</option>';
  }
  return (
    '<select class="car-resp-select" onchange="atribuirResponsavelCarrinho(\'' +
    safeId +
    '\',this.value)">' +
    opts +
    '</select>'
...
function marcarCarrinhoPerdido(checkoutId) {
  ...
  carrinhosAbandonados[idx] = Object.assign(
    {},
    normalizeCarrinhoAbandonado(carrinhosAbandonados[idx]),
    { perdido: true, perdido_em: new Date().toISOString() },
  );
  safeSetItem('crm_carrinhos_abandonados', JSON.stringify(carrinhosAbandonados));
  if (supaConnected && supaClient)
    upsertCarrinhosAbandonadosToSupabase([carrinhosAbandonados[idx]]).catch(() => {});
...
function toggleCarrinhoSelection(id, checked) {
  const cid = String(id || '');
  if (checked) selectedCarrinhos.add(cid);
  else selectedCarrinhos.delete(cid);
...
function openWaModalCarrinho(checkoutId) {
  ...
  const tpls = [
    ...
    },
    {
      ...
      texto: [
        `Oi ${nome}!`,
        'Passando pra te ajudar a finalizar seu pedido.',
        prodTxt ? `Itens: ${prodTxt}` : '',
        valorTxt ? `Total: ${valorTxt}` : '',
        link ? `Link pra finalizar: ${link}` : 'Posso te mandar o link pra finalizar rapidinho 👇',
      ]
        .filter(Boolean)
        .join(' '),
    },
    {
      ...
      texto: [
        `Oi ${nome}!`,
        'Só passando pra lembrar do seu carrinho que ficou por aqui.',
        prodTxt ? `${prodTxt}` : '',
        valorTxt ? `Total: ${valorTxt}` : '',
        prio.id === 'alta'
          ? 'Se precisar de ajuda pra fechar, me fala 🙂'
          : 'Se fizer sentido pra você, consigo verificar o que posso fazer!',
        link ? `Link: ${link}` : '',
      ]
        .filter(Boolean)
        .join(' '),
  ...
  const recIdx =
    etapa.id === 'ajuda' ? 0 : etapa.id === 'link' ? 1 : etapa.id === 'incentivo' ? 2 : 0;
  ...
  if (nameEl)
    nameEl.textContent = `${nome}${valorTxt ? ' · ' + valorTxt : ''}${prodTxt ? ' · ' + prodTxt : ''}`;
  ...
  window.sendWa = function () {
    ...
    const nowIso = new Date().toISOString();
    const idx2 = (carrinhosAbandonados || []).findIndex(
      (x) => String(x?.checkout_id || '') === cid,
    );
    if (idx2 >= 0) {
      carrinhosAbandonados[idx2] = Object.assign(
        {},
        normalizeCarrinhoAbandonado(carrinhosAbandonados[idx2]),
        { last_etapa_enviada: etapaId, last_mensagem_at: nowIso },
      );
      safeSetItem('crm_carrinhos_abandonados', JSON.stringify(carrinhosAbandonados));
      if (supaConnected && supaClient)
        upsertCarrinhosAbandonadosToSupabase([carrinhosAbandonados[idx2]]).catch(() => {});
      if (typeof window.renderCarrinhosAbandonados === 'function')
        window.renderCarrinhosAbandonados();
    }
...
function openCarrinhoBatchWa() {
  ...
  const selected = (carrinhosAbandonados || []).filter((c) =>
    selectedCarrinhos.has(String(c.checkout_id || '')),
  );
  if (!selected.length) {
    toast('Nenhum carrinho selecionado.', 'warning');
    return;
  }
...
function batchCopyAllCarrinhos() {
  ...
  if (!items.length) {
    toast('Nenhuma mensagem para copiar.', 'warning');
    return;
  }
  ...
  if (navigator.clipboard) {
    navigator.clipboard
      .writeText(all)
      .then(() => toast(`${items.length} mensagens copiadas!`, 'success'))
      .catch(() => {
        fallbackCopy(all);
        toast(`${items.length} mensagens copiadas!`, 'success');
      });
  } else {
    fallbackCopy(all);
    toast(`${items.length} mensagens copiadas!`, 'success');
...
function renderCarrinhosAbandonados() {
  var el = document.getElementById('carrinhos-list');
  if (!el) return;
  try {
    carrinhosAbandonados =
      safeJsonParse('crm_carrinhos_abandonados', []) || carrinhosAbandonados || [];
  } catch (_e) {}
  var q = String((document.getElementById('car-search') || {}).value || '')
    .trim()
    .toLowerCase();
  ...
  var enriched = []
    ...
    .map(function (c) {
      ...
      return Object.assign({}, c, {
        score_calc: score,
        tempo_min: mins,
        tempo_label: fmtTempoDesde(mins),
        etapa_id: etapa.id,
        etapa_label: etapa.label,
        prioridade_id: prio.id,
        prioridade_label: prio.label,
        last_tempo_label: lastMins == null ? '' : fmtTempoDesde(lastMins),
      });
    });
  ...
  var list = enriched
    .filter(function (c) {
      ...
      if (
        st === 'quentes' &&
        (c.recuperado || c.perdido || c.tempo_min == null || c.tempo_min >= 120)
      )
        return false;
      if (resp && (c.responsavel || '') !== resp) return false;
      if (q) {
    ...
    .sort(function (a, b) {
      ...
      if ((b.score_calc || 0) !== (a.score_calc || 0))
        return (b.score_calc || 0) - (a.score_calc || 0);
  ...
  el.innerHTML =
    ...
    list
      .map(function (c) {
        ...
        var contato =
          [c.telefone ? escapeHTML(fmtPhone(c.telefone)) : '', c.email ? escapeHTML(c.email) : '']
            .filter(Boolean)
            .join(' · ') || '—';
        ...
        var resumo = itens
          .slice(0, 2)
          .map(function (it) {
            return String(it?.nome || it?.title || it?.descricao || it?.name || '').trim();
          })
          .filter(Boolean)
          .join(', ');
        var subtitle = resumo
          ...
          : '';
        var lastInfo =
          !c.recuperado && !c.perdido && c.last_etapa_enviada
            ? '<div style="font-size:10px;color:var(--text-3);margin-top:2px">Última: ' +
              escapeHTML(c.last_etapa_enviada) +
              (c.last_tempo_label ? ' · ' + escapeHTML(c.last_tempo_label) : '') +
              '</div>'
            : '';
        var badge = pipelineBadge(c);
        var btnWa =
          !c.recuperado && !c.perdido && rawPhone(c.telefone || '')
            ? '<button class="opp-mini-btn" onclick="openWaModalCarrinho(\'' +
              safeId +
              '\')">💬 WA</button> '
            : '';
        var btnHist =
          '<button class="opp-mini-btn" onclick="openCarrinhoHistorico(\'' +
          safeId +
          '\')" title="Ver histórico de interações">📋</button> ';
        var btnPer =
          !c.recuperado && !c.perdido
            ? '<button class="opp-mini-btn" style="color:var(--red,#f87171)" onclick="marcarCarrinhoPerdido(\'' +
              safeId +
              '\')">✗</button>'
            : '';
        var prioIcon = c.prioridade_id === 'alta' ? '🔥' : c.prioridade_id === 'media' ? '⚡' : '';
        var nextAcao = c.recuperado
          ? '—'
          : c.perdido
            ? '—'
            : c.etapa_label
              ? prioIcon + ' ' + c.etapa_label
              : '—';
        var checked = selectedCarrinhos.has(String(c.checkout_id || ''));
        return (
          '<tr>' +
          '<td><input type="checkbox" class="car-check car-row-check" data-id="' +
          escapeHTML(String(c.checkout_id || '')) +
          '" ' +
          (checked ? 'checked' : '') +
          ' onchange="toggleCarrinhoSelection(\'' +
          safeId +
          '\',this.checked)"/></td>' +
          '<td class="chiva-table-mono" style="white-space:nowrap">' +
          escapeHTML(c.tempo_label || '—') +
          '</td>' +
          '<td><div style="font-weight:800;color:var(--text)">' +
          name +
          '</div>' +
          subtitle +
          '<div style="font-size:10px;color:var(--text-3);margin-top:2px">' +
          ...
          lastInfo +
          '</td>' +
          '<td style="text-align:right" class="chiva-table-mono">' +
          escapeHTML(String(Math.round(c.score_calc || 0))) +
          '</td>' +
          '<td style="text-align:right" class="chiva-table-mono">' +
          escapeHTML(fmtBRL(c.valor || 0)) +
          '</td>' +
          '<td style="font-size:11px;color:var(--text-2)">' +
          escapeHTML(nextAcao) +
          '</td>' +
          '<td>' +
          buildRespSelect(safeId, c.responsavel) +
          '</td>' +
...
function setComTab(tab) {
  ...
  if (tab === 'pedidos') {
    renderComPedidos();
    setTimeout(renderChartsCom, 100);
  }
...
function abrirModalCampanha(id) {
  ...
  if (id) {
    var c = allCampanhas.find(function (x) {
      return x.id === id;
    });
    if (!c) return;
...
function salvarCampanha() {
  ...
  var obj = {
    ...
    nome: document.getElementById('camp-nome').value.trim(),
    canal: document.getElementById('camp-canal').value,
    tipo: document.getElementById('camp-tipo').value,
    inicio: parseDateToIso(document.getElementById('camp-inicio').value),
    fim: parseDateToIso(document.getElementById('camp-fim').value),
    oferta: document.getElementById('camp-oferta').value.trim(),
    budget: parseFloat(document.getElementById('camp-budget').value) || 0,
    meta: parseFloat(document.getElementById('camp-meta').value) || 0,
    status: document.getElementById('camp-status').value,
    desc: document.getElementById('camp-desc').value.trim(),
  };
  if (!obj.nome) {
    toast('⚠️ Informe o nome da campanha');
    return;
  }
  if (id) {
    var idx = allCampanhas.findIndex(function (x) {
      return x.id === parseInt(id);
    });
    if (idx >= 0) allCampanhas[idx] = obj;
  } else allCampanhas.push(obj);
  saveCampanhas();
  renderCampanhas();
  renderComKpis();
...
function kpiCard(title, value, subtitle, color) {
  return (
    '<div class="car-kpi-card" style="--i:0">' +
    '<div class="car-kpi-value" style="color:' +
    escapeHTML(String(color || 'var(--text)')) +
    '">' +
    escapeHTML(String(value ?? '')) +
    '</div>' +
    '<div class="car-kpi-label">' +
...
function miniKpi(label, value, color) {
  return (
    ...
    escapeHTML(String(value ?? '')) +
    '</div>' +
    '<div style="font-size:9px;font-weight:700;color:var(--text-3);text-transform:uppercase;letter-spacing:.04em;margin-top:2px">' +
...
let allEventos = safeJsonParse('crm_eventos', null) || [
  {
    id: 1,
    titulo: 'Degustação Shopping Iguatemi',
    tipo: 'degustacao',
    data: '2026-03-22',
    hora: '10:00',
    local: 'Shopping Iguatemi — BH',
    responsavel: 'Ana',
    custo: 350,
    amostras: 120,
    conversoes: 18,
    receita: 1620,
    obs: 'Ótima receptividade sabor chocolate',
  },
  {
    id: 2,
    titulo: 'Feira Natural Expo',
    tipo: 'feira',
    data: '2026-04-12',
    hora: '09:00',
    local: 'Expo Center — SP',
    responsavel: 'Carlos',
    custo: 1200,
    amostras: 250,
    conversoes: 0,
    receita: 0,
    obs: 'Montar estande 3x3',
  },
  {
    id: 3,
    titulo: 'Live com Nutricionista',
    tipo: 'live',
    data: '2026-03-28',
    hora: '19:00',
    local: 'Instagram @chivafit',
    responsavel: 'Marketing',
    custo: 0,
    amostras: 0,
    conversoes: 0,
    receita: 0,
    obs: 'Tema: proteína na dieta feminina',
  },
  {
    id: 4,
    titulo: 'Degustação Academia FitLife',
    tipo: 'degustacao',
    data: '2026-04-05',
    hora: '07:00',
    local: 'Academia FitLife — BH',
    responsavel: 'Ana',
    custo: 150,
    amostras: 60,
    conversoes: 0,
...
function saveEventos() {
  localStorage.setItem('crm_eventos', JSON.stringify(allEventos));
}

function renderMarcaKpis() {
  var el = document.getElementById('marca-kpis');
  if (!el) return;
  var degust = allEventos.filter(function (e) {
    return e.tipo === 'degustacao';
  });
  var tA = degust.reduce(function (s, e) {
    return s + (e.amostras || 0);
  }, 0);
  var tC = degust.reduce(function (s, e) {
    return s + (e.conversoes || 0);
  }, 0);
  var tR = degust.reduce(function (s, e) {
    return s + (e.receita || 0);
  }, 0);
  var taxa = tA > 0 ? Math.round((tC / tA) * 100) : 0;
  el.innerHTML =
    kpiCard('Eventos', allEventos.length, 'cadastrados', 'var(--text)') +
    kpiCard('Degustações', degust.length, 'realizadas', 'var(--amber)') +
    kpiCard('Amostras', tA, 'distribuídas', 'var(--blue)') +
    kpiCard('Taxa Conv.', taxa + '%', tC + ' vendas', 'var(--green)') +
    kpiCard(
      'Receita Trade',
      'R$' + tR.toLocaleString('pt-BR', { minimumFractionDigits: 0 }),
      'gerada',
      'var(--green)',
    );
}
...
function renderCalendario() {
  ...
  var hoje = new Date();
  var evMes = allEventos.filter(function (e) {
    var d = new Date(e.data + 'T12:00:00');
    return d.getMonth() === calMesAtual && d.getFullYear() === calAnoAtual;
  });
  var diasEv = new Set(
    evMes.map(function (e) {
      return parseInt(e.data.split('-')[2]);
    }),
  );
  var h = dias
    .map(function (d) {
      return '<div class="cal-header-day">' + d + '</div>';
    })
    .join('');
  for (var i = 0; i < prim; i++)
    h +=
      '<div class="cal-day outro-mes"><div class="cal-day-num" style="color:var(--text-4)">' +
      new Date(calAnoAtual, calMesAtual, -(prim - i - 1)).getDate() +
      '</div></div>';
  for (var d = 1; d <= ult; d++) {
    var isH =
      d === hoje.getDate() && calMesAtual === hoje.getMonth() && calAnoAtual === hoje.getFullYear();
...
function filtrarDia(dia) {
  filtroDia = filtroDia === dia ? null : dia;
  renderEventosLista();
}
...
function renderEventosLista() {
  ...
  var list = [].concat(allEventos).sort(function (a, b) {
  ...
  if (filtroDia !== null) {
    list = list.filter(function (e) {
      var d = new Date(e.data + 'T12:00:00');
      return (
        d.getMonth() === calMesAtual &&
        d.getFullYear() === calAnoAtual &&
        parseInt(e.data.split('-')[2]) === filtroDia
      );
    });
  } else {
    list = list.filter(function (e) {
...
function renderDegustacoes() {
  var el = document.getElementById('degustacoes-list');
  if (!el) return;
  var list = allEventos.filter(function (e) {
    return e.tipo === 'degustacao';
  });
  if (!list.length) {
    el.innerHTML = '<div class="empty">Nenhuma degustação cadastrada</div>';
    return;
  }
  el.innerHTML = list
    .map(function (e) {
      ...
      return (
        '<div class="degust-card"><div style="display:flex;justify-content:space-between;align-items:start;flex-wrap:wrap;gap:12px"><div><div style="font-size:13px;font-weight:700;margin-bottom:3px">' +
        escapeHTML(e.titulo) +
        '</div><div style="font-size:10px;color:var(--text-3)">' +
        escapeHTML(fmtDate(e.data)) +
        ' · ' +
        escapeHTML(e.local) +
        ' · ' +
        escapeHTML(e.responsavel) +
        '</div></div><button onclick="abrirModalEvento(' +
        e.id +
        ')" style="background:none;border:1px solid var(--border);border-radius:var(--r-md);padding:3px 10px;color:var(--text-2);font-size:10px;font-weight:700;cursor:pointer;font-family:var(--font)">Editar</button></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(90px,1fr));gap:8px;margin-top:12px">' +
        miniKpi('🧪 Amostras', e.amostras || 0, 'var(--blue)') +
        miniKpi('✅ Conversões', e.conversoes || 0, 'var(--green)') +
        miniKpi('📈 Conv.%', taxa + '%', 'var(--indigo-hi)') +
        miniKpi('💰 Custo', 'R$' + (e.custo || 0), 'var(--red)') +
        miniKpi('💵 Receita', 'R$' + (e.receita || 0), 'var(--green)') +
        (roi !== null
          ? miniKpi('🚀 ROI', roi + '%', parseInt(roi) >= 0 ? 'var(--green)' : 'var(--red)')
          : '') +
        '</div>' +
        (e.obs
          ? '<div style="font-size:11px;color:var(--text-2);margin-top:12px;padding-top:12px;border-top:1px solid var(--border-sub);font-style:italic">' +
            escapeHTML(e.obs) +
            '</div>'
          : '') +
        '</div>'
...
function renderMarcaResultados() {
  var el = document.getElementById('marca-resultados');
  if (!el) return;
  var degust = allEventos.filter(function (e) {
    return e.tipo === 'degustacao';
  });
  ...
  var tCusto = allEventos.reduce(function (s, e) {
    return s + (e.custo || 0);
  }, 0);
  var tR = allEventos.reduce(function (s, e) {
    return s + (e.receita || 0);
  }, 0);
  ...
  // Por tipo
  var tipoMap = {
    degustacao: { label: '🍫 Degustação' },
    feira: { label: '🏪 Feira' },
    evento: { label: '🎪 Evento' },
    reuniao: { label: '🤝 Reunião' },
    live: { label: '📱 Live' },
  };
  var tipoRows = Object.keys(tipoMap)
    .map(function (t) {
      var evs = allEventos.filter(function (e) {
        return e.tipo === t;
      });
      if (!evs.length) return '';
      var custo = evs.reduce(function (s, e) {
        return s + (e.custo || 0);
      }, 0);
      var receita = evs.reduce(function (s, e) {
        return s + (e.receita || 0);
      }, 0);
      ...
      return (
        '<tr>' +
        '<td>' +
        escapeHTML(tipoMap[t].label) +
        '</td>' +
        '<td style="text-align:center">' +
        evs.length +
        '</td>' +
        '<td style="text-align:right">' +
        escapeHTML('R$' + custo.toLocaleString('pt-BR')) +
        '</td>' +
        '<td style="text-align:right">' +
        escapeHTML('R$' + receita.toLocaleString('pt-BR')) +
        '</td>' +
        '<td style="text-align:right;font-weight:700;color:' +
        (roiT !== null && parseInt(roiT) >= 0 ? 'var(--green)' : 'var(--red)') +
        '">' +
        escapeHTML(roiT !== null ? roiT + '%' : '—') +
        '</td>' +
        '</tr>'
      );
    })
  ...
  el.innerHTML =
    ...
    miniKpi('Total Amostras', tA, 'var(--blue)') +
    miniKpi('Conversões', tC, 'var(--green)') +
    miniKpi('Taxa Conv.', taxa + '%', 'var(--indigo-hi)') +
    miniKpi('Custo Total', 'R$' + tCusto.toLocaleString('pt-BR'), 'var(--red)') +
    miniKpi('Receita Total', 'R$' + tR.toLocaleString('pt-BR'), 'var(--green)') +
    miniKpi('ROI Geral', roi + '%', parseInt(roi) >= 0 ? 'var(--green)' : 'var(--red)') +
    '</div>' +
    '</div>' +
    (tipoRows
...
function abrirModalEvento(id) {
  ...
  if (id) {
    var e = allEventos.find(function (x) {
      return x.id === id;
    });
    if (!e) return;
...
function salvarEvento() {
  ...
  var obj = {
    ...
    custo: parseFloat(document.getElementById('ev-custo').value) || 0,
    amostras: parseInt(document.getElementById('ev-amostras').value) || 0,
    conversoes: parseInt(document.getElementById('ev-conversoes').value) || 0,
    receita: parseFloat(document.getElementById('ev-receita').value) || 0,
    obs: document.getElementById('ev-obs').value.trim(),
  };
  if (!obj.titulo) {
    toast('⚠️ Informe o título', 'warning');
    return;
  }
  if (!obj.data) {
    toast('⚠️ Informe a data do evento', 'warning');
    return;
  }
  if (id) {
    var idx = allEventos.findIndex(function (x) {
      return x.id === parseInt(id);
    });
    if (idx >= 0) allEventos[idx] = obj;
  } else allEventos.push(obj);
  saveEventos();
  renderCalendario();
  renderDegustacoes();
  renderMarcaResultados();
  renderMarcaKpis();
...
function renderChartEstoque() {
  ...
  var sorted = []
    .concat(allInsumos)
    .sort(function (a, b) {
      return getEstPct(b) - getEstPct(a);
    })
    .slice(0, 10);
  window._chartEstoque = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: sorted.map(function (i) {
        return i.nome.length > 14 ? i.nome.slice(0, 12) + '…' : i.nome;
      }),
      datasets: [
        {
          label: 'Nível de Estoque',
          data: sorted.map(function (i) {
            return Math.min(100, getEstPct(i));
          }),
...
function renderChartsCom() {
  ...
  if (cCtx && cCtx.getContext) {
    ...
    var cData = {};
    getComPedidosBase().forEach(function (p) {
      cData[p.canal] = (cData[p.canal] || 0) + p.valor;
    });
    var sorted = Object.entries(cData).sort(function (a, b) {
      return b[1] - a[1];
    });
    var cColors = {
      shopee: 'var(--shopee)',
    ...
    window._chartComCanal = new Chart(cCtx, {
      ...
      data: {
        labels: sorted.map(function (e) {
          return normalizeCanal(e[0]).nome;
        }),
        datasets: [
          {
  ...
  if (sCtx && sCtx.getContext) {
    ...
    var sData = {};
    getComPedidosBase().forEach(function (p) {
      sData[p.status] = (sData[p.status] || 0) + 1;
    });
    var sColors = {
      novo: '#60a5fa',
      separando: '#fbbf24',
    ...
    var sorted2 = Object.entries(sData).sort(function (a, b) {
      return b[1] - a[1];
    });
    window._chartComStatus = new Chart(sCtx, {
      type: 'bar',
      data: {
        labels: sorted2.map(function (e) {
          return sLabels[e[0]] || e[0];
        }),
        datasets: [
          {
            data: sorted2.map(function (e) {
              return e[1];
            }),
