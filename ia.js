...
export function getTodaySalesActions(ctx) {
  if (Array.isArray(ctx.customerIntelligence) && ctx.customerIntelligence.length) {
    return [...ctx.customerIntelligence]
      .sort((a, b) => {
  ...
  if (Array.isArray(ctx.customerIntel) && ctx.customerIntel.length) {
    return [...ctx.customerIntel]
      .sort((a, b) => {
        const pa = a.action_priority ?? 99;
        const pb = b.action_priority ?? 99;
        if (pa !== pb) return pa - pb;
        const ra = a.risco_churn ?? 0;
        const rb = b.risco_churn ?? 0;
...
function normalizeIntelRow(ctx, row) {
  ...
  const ticket_medio =
    row.ticket_medio ??
    (safeNum(valor_total, 0) && safeNum(total_pedidos, 0)
      ? safeNum(valor_total, 0) / Math.max(1, safeNum(total_pedidos, 0))
      : 0);
...
function isVipRow(intel) {
  ...
  const action = intel.next_best_action || '';
  return (
    action === 'tratamento_vip' || safeNum(intel.valor_total, 0) >= 650 || intel.score_norm >= 90
  );
}
...
function scheduleCustomerIntelligenceSync(ctx) {
  ...
  customerIntelSyncTimer = setTimeout(() => {
    ...
    upsertCustomerIntelligenceToSupabase(ctx, ctx.customerIntelligence).catch((e) =>
      console.warn('customerIntel sync:', e.message),
    );
  }, 5000);
}
...
async function upsertCustomerIntelligenceToSupabase(ctx, intel) {
  ...
  let cliRows = [];
  try {
    const { data, error } = await ctx.supaClient
      .from('v2_clientes')
    ...
    .map((c) => {
      ...
      if (!customerId) {
        ...
        if (docDigits.length === 11 || docDigits.length === 14)
          customerId = byDoc[docDigits] || null;
      }
  ...
  const buildBatch = (mode, batch) => {
    ...
    if (mode === 'score_only')
      return batch.map((r) => ({ cliente_id: r.cliente_id, score_final: r.score_final }));
    return batch;
  };
  const tryModes = customerIntelUpsertMode
    ? [customerIntelUpsertMode, 'full', 'no_updated_at', 'score_only']
    : ['full', 'no_updated_at', 'score_only'];
  ...
  for (let i = 0; i < rowsBase.length; i += 100) {
    ...
    for (let m = 0; m < tryModes.length; m++) {
      ...
      const { error } = await ctx.supaClient
...
export function computeCustomerIntelligence(ctx) {
  ...
  Object.values(map).forEach((c) => {
    ...
    const score_final = Math.round(recenciaScore * 0.4 + freqScore * 0.3 + valorScore * 0.3);
...
export function formatNextBestAction(a) {
  switch (a) {
    case 'tratamento_vip':
      return 'Tratamento VIP';
    case 'oferta_kit':
      return 'Oferta de kit';
    case 'oferecer_assinatura':
      return 'Oferecer assinatura';
    case 'sugerir_recompra':
      return 'Sugerir recompra';
    case 'reativar_sem_desconto':
      return 'Reativar (sem desconto)';
    case 'reativacao_com_cupom':
      return 'Reativação com cupom';
    case 'nutrir_cliente':
      return 'Nutrir cliente';
    case 'nao_acionar':
      return 'Não acionar';
    default:
      return a || '-';
...
export function renderIADashboard(ctx) {
  ...
  if (!hasAnyData) {
    sumEl.innerHTML = emptyState(
      '🧠',
      'Nenhum dado de inteligência ainda',
      'Importe pedidos e rode a inteligência de clientes.',
    );
    riskEl.innerHTML = '';
    rebuyEl.innerHTML = '';
    vipEl.innerHTML = '';
  ...
  const baseList = (localList.length ? localList : intelList)
    .map((r) => normalizeIntelRow(ctx, r))
    .filter(Boolean);
  const total = baseList.length;
  const topActions = getTodaySalesActions(ctx)
    .map((r) => normalizeIntelRow(ctx, r))
    .filter(Boolean);
  ...
  const emRisco = baseList
    .filter(isRiskRow)
    .sort((a, b) => (a.action_priority ?? 99) - (b.action_priority ?? 99));
  const recompra = baseList
    .filter(isRebuyRow)
    .sort((a, b) => (a.action_priority ?? 99) - (b.action_priority ?? 99));
  const vips = baseList.filter(isVipRow).sort((a, b) => (b.score_norm ?? 0) - (a.score_norm ?? 0));
  const assinatura = baseList
  ...
  const resolveCustomer = (cidRaw) => {
    ...
    const byId = Array.isArray(ctx.allCustomers)
      ? ctx.allCustomers.find((x) => String(x.id || '') === cid)
      : null;
    if (byId) return byId;
    const byDoc =
      digits && Array.isArray(ctx.allCustomers)
        ? ctx.allCustomers.find((x) => String(x.doc || '').replace(/\D/g, '') === digits)
        : null;
    return byDoc || null;
  };
...
function parseCommercialAIResponse(data) {
  ...
  return {
    diagnostico: [],
    acao_recomendada: '',
    valor_potencial_estimado: null,
    mensagens: msgs.slice(0, 5),
  };
}
...
function renderActionConsole(ctx, clienteId, contextoTipo, opts) {
  ...
  const cid = String(clienteId || '');
  const intelRaw =
    ctx.customerIntelligence.find((c) => String(c.cliente_id) === cid) ||
  ...
  const baseMsg = (
    intel.suggested_whatsapp_message ||
    generateWhatsAppMessage(ctx, intel, rawCli) ||
    ''
  ).trim();
...
function openWhatsAppForCustomerWithText(ctx, clienteId, text) {
  ...
  const nowIso = new Date().toISOString();
  try {
    intelRaw.last_whatsapp_at = nowIso;
  } catch (_e) {}
...
export async function gerarMensagemIA(ctx, clienteId, contextoTipo) {
  ...
  const intelRaw =
    ...
    ctx.customerIntel.find((c) => String(c.cliente_id) === cid);
  if (!intelRaw) {
    ctx.toast('⚠ Cliente não encontrado na inteligência');
    return;
  }
  const intel = normalizeIntelRow(ctx, intelRaw);
  const cliMap = ctx.buildCli(ctx.allOrders);
  const rawCli = cliMap[intel.cliente_id] || null;
  const pedidosCli = ctx.allOrders
    .filter((o) => String(o.cliente_id || o.cliente_uuid || ctx.cliKey(o)) === cid)
    .slice(0, 30);
  ...
  if (variantsEl)
    variantsEl.innerHTML = `<div class="ai-thinking"><div class="ai-dots"><span></span><span></span><span></span></div>Gerando variações de mensagem...</div>`;
...
Personalização: use o primeiro nome do cliente quando possível (${firstName || 'N/A'}).
  ...
  try {
    const resp = await fetch(ctx.getSupaFnBase() + '/ia-commercial', {
      method: 'POST',
    ...
    if (!resp.ok) {
      ...
      throw new Error(txt || 'Erro na IA comercial');
    }
    ...
    const diagHtml = diag.length
      ? `<ul class="ia-ai-bullets">${diag.map((x) => `<li>${ctx.escapeHTML(String(x))}</li>`).join('')}</ul>`
      : '';
      ...
      .map((m, i) => {
        ...
        <div class="ia-ai-msg">
          <div class="ia-ai-msg-top">
            ...
            <div class="ia-ai-msg-meta">${[objetivo, tom]
              .filter(Boolean)
              .map((x) => `<span class="ia-chip">${ctx.escapeHTML(x)}</span>`)
              .join('')}</div>
          </div>
    ...
    if (variantsEl)
      variantsEl.innerHTML = `<div class="ia-error">⚠ ${ctx.escapeHTML(e.message || 'Erro')}</div>`;
  }
}
...
export function copyWhatsAppMessageForCustomer(ctx, clienteId) {
  const intel =
    ctx.customerIntelligence.find((c) => String(c.cliente_id) === String(clienteId)) ||
  ...
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard
      .writeText(msg)
      .then(() => ctx.toast('📋 Mensagem copiada!'))
      .catch(() => {
...
function buildDataSummary(ctx) {
  ...
  const vips = clis
    .filter((c) => {
      ...
      return sc.status === 'vip' || sc.ltv >= 650;
    })
    .filter((c) => !ctx.isCNPJ(c.doc));

  const canalStats = {};
  ctx.allOrders.forEach((o) => {
    const ch = ctx.detectCh(o);
    if (!canalStats[ch]) canalStats[ch] = { pedidos: 0, total: 0, clientes: new Set() };
    canalStats[ch].pedidos++;
    canalStats[ch].total += ctx.val(o);
    canalStats[ch].clientes.add(ctx.cliKey(o));
  });
  const canalSummary = Object.entries(canalStats)
    .map(
      ([ch, s]) =>
        `${ctx.CH[ch]}: ${s.pedidos} pedidos, ${ctx.fmtBRL(s.total)}, ${s.clientes.size} clientes únicos`,
    )
    .join(' | ');
  ...
  ctx.allOrders.forEach((o) =>
    (o.itens || []).forEach((it) => {
      const k = it.descricao || it.codigo || '?';
      if (!prodMap[k]) prodMap[k] = { t: 0, qty: 0, clis: new Set() };
      prodMap[k].t += (parseFloat(it.valor) || 0) * (parseFloat(it.quantidade) || 1);
      prodMap[k].qty += parseFloat(it.quantidade) || 1;
      prodMap[k].clis.add(ctx.cliKey(o));
    }),
  );
  const topProds = Object.entries(prodMap)
    .sort((a, b) => b[1].t - a[1].t)
    .slice(0, 15)
    .map(([n, v]) => `${n}: ${v.qty} un, ${ctx.fmtBRL(v.t)}, ${v.clis.size} clientes`)
    .join('\n');
  ...
  ctx.allOrders.forEach((o) => {
    ...
    bm[k] = (bm[k] || 0) + ctx.val(o);
  });
  const monthlyTrend = Object.entries(bm)
    .sort((a, b) => a[0].localeCompare(b[0]))
    .slice(-12)
    .map(([k, v]) => `${k}: ${ctx.fmtBRL(v)}`)
    .join(', ');
  ...
  const topClis = clis
    ...
    .sort(
      (a, b) =>
        b.orders.reduce((s, o) => s + ctx.val(o), 0) - a.orders.reduce((s, o) => s + ctx.val(o), 0),
    )
    .slice(0, 10)
    .map((c) => {
      const sc = ctx.calcCliScores(c);
...
export async function runAI(ctx, type) {
  if (!ctx.allOrders.length) {
    ctx.toast('⚠ Importe dados primeiro');
    return;
  }
  ...
  if (resultEl)
    resultEl.innerHTML = `<div class="ai-thinking"><div class="ai-dots"><span></span><span></span><span></span></div>Analisando dados reais...</div>`;
  ...
  try {
    const response = await fetch(ctx.getSupaFnBase() + '/ia-claude', {
      ...
      body: JSON.stringify({
        ...
        system:
          'Você é um especialista em CRM e retenção de clientes para e-commerce brasileiro, especialmente marcas de suplementos/nutrição. Responda sempre em português, seja direto, use dados reais fornecidos, e dê insights acionáveis. Use bullet points e seja conciso mas impactante.',
      }),
    });
    if (!response.ok) {
      const errText = await response.text();
      throw new Error(errText || 'Erro na função IA');
    }
    const data = await response.json();
    const text = (data.text || '').trim();
    const safeText = ctx.escapeHTML(text);
    if (resultEl)
      resultEl.innerHTML = `<div class="ai-result">${safeText.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br>')}</div>`;
    try {
      if (ctx.supaConnected && ctx.supaClient) {
        await ctx.supaClient.from('v2_insights').insert({
          tipo: type,
          conteudo: text,
          gerado_por: ctx.selectedUser,
    ...
    if (resultEl)
      resultEl.innerHTML = `<div style="color:var(--red);font-size:12px">⚠ ${safeErr}</div>`;
  } finally {
    if (btn) {
      btn.disabled = false;
