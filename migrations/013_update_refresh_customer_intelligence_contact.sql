  ...
  IF has_pedidos_bling AND has_pedidos_bling_raw THEN
    WITH ords AS (
      SELECT
        ...
        regexp_replace(COALESCE(pedidos_bling.raw->'contato'->>'telefone', ''), '\D', '', 'g') AS telefone,
        regexp_replace(COALESCE(pedidos_bling.raw->'contato'->>'celular', ''), '\D', '', 'g') AS celular,
    ...
    agg AS (
      SELECT
        ...
        max(celular) FILTER (WHERE celular IS NOT NULL AND celular <> '') AS celular,
      ...
      FROM ords
      WHERE COALESCE(NULLIF(doc_digits, ''), NULLIF(email, ''), NULLIF(telefone, ''), NULLIF(celular, '')) IS NOT NULL
      GROUP BY 1
    )
    ...
    SELECT
      ...
      CASE WHEN position('@' in a.doc) > 0 THEN a.doc ELSE NULLIF(a.email, '') END,
      CASE WHEN a.doc ~ '^[0-9]{10,}$' THEN a.doc ELSE NULLIF(a.telefone, '') END,
      NULLIF(a.celular, ''),
      a.primeiro_pedido,
      a.ultimo_pedido,
      a.total_pedidos,
    ...
    ON CONFLICT (doc) DO UPDATE SET
      ...
      celular = COALESCE(EXCLUDED.celular, public.v2_clientes.celular),
