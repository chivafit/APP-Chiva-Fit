...
function kpiCard(title, value, subtitle, color) {
  if (typeof globalThis.kpiCard === 'function')
    return globalThis.kpiCard(title, value, subtitle, color);
  return '';
}
...
const PRECO_KG = {
  ...
  'Sal Himalaia': 8.15,
  'Óleo de Coco': 108.0,
  'Extrato de Baunilha': 778.0,
  Stevia: 560.0,
  TCM: 131.0,
  'Quinoa em Flocos': 32.9,
  'Proteína de Ervilha': 68.9,
  Canela: 6.0,
};
...
const RECEITAS_REAIS = {
  'Granola Cranberry 250g': {
    tamanho: '250g',
    categoria: 'cranberry',
    insumos: {
      'Semente de Girassol': 58,
      Cranberry: 40,
      'Flocos de Milho': 38,
      'Castanha de Caju': 30,
  ...
  'Granola Cranberry 30g': {
    tamanho: '30g',
    categoria: 'cranberry',
    insumos: {
      'Semente de Girassol': 4,
      Cranberry: 4,
      'Flocos de Milho': 8,
      'Castanha de Caju': 3,
  ...
  'Granola Tradicional 30g': {
    tamanho: '30g',
    categoria: 'tradicional',
    insumos: {
      'Semente de Girassol': 1,
      'Flocos de Milho': 11,
      'Uva Passa': 5,
      'Castanha de Caju': 2,
      'Semente de Abóbora': 5,
      'Linhaça Dourada': 2,
      'Chips de Coco': 1,
      Stevia: 1,
      Canela: 1,
  ...
  'Granola Low Carb 200g': {
    tamanho: '200g',
    categoria: 'lowcarb',
    insumos: {
      'Semente de Girassol': 35,
      'Semente de Abóbora': 30,
      'Castanha de Caju': 15,
      'Linhaça Dourada': 30,
      Amêndoas: 45,
  ...
  'Granola Low Carb 30g': {
    tamanho: '30g',
    categoria: 'lowcarb',
    insumos: {
      'Semente de Girassol': 3,
      'Semente de Abóbora': 4,
      'Castanha de Caju': 2,
      'Linhaça Dourada': 4,
      Amêndoas: 6,
      Chia: 3,
  ...
  'Granola Protein 200g': {
    tamanho: '200g',
    categoria: 'protein',
    insumos: {
      'Quinoa em Flocos': 32,
      'Proteína de Ervilha': 35,
      'Semente de Abóbora': 30,
      'Castanha de Caju': 16,
      Amêndoas: 10,
      Chia: 10,
      'Linhaça Dourada': 20,
      'Semente de Girassol': 34,
      'Castanha do Pará': 8,
      Stevia: 1,
      'Óleo de Coco': 1,
      Canela: 1,
  ...
  'Granola Protein 30g': {
    ...
    insumos: {
      'Quinoa em Flocos': 6,
      'Proteína de Ervilha': 7,
      'Semente de Abóbora': 4,
      'Castanha de Caju': 2,
      Amêndoas: 1,
      Chia: 1,
      'Linhaça Dourada': 2,
      'Semente de Girassol': 2,
      'Castanha do Pará': 1,
      Stevia: 1,
      'Óleo de Coco': 1,
      Canela: 1,
...
let allInsumos =
  safeJsonParse('crm_insumos', null) ||
  Object.keys(PRECO_KG).map(function (nome, i) {
    var base = {
      id: String(i + 1),
      nome: nome,
      unidade: 'kg',
...
function migrateLegacyOrdensToProducao() {
  ...
  var hasLegacy =
    ...
    legacy.some(function (o) {
      return o && Array.isArray(o.itens) && o.itens.length;
    });
  if (!hasLegacy) return [];
  var next = [];
...
let allProdutosReceitas =
  safeJsonParse('crm_receitas_produtos_produtos', null) || Object.keys(RECEITAS_REAIS);
...
function saveInsumos() {
  ...
  if (typeof globalThis.syncInsumosToSupabase === 'function')
    globalThis.syncInsumosToSupabase(allInsumos);
}
function saveOrdens() {
  localStorage.setItem('crm_ordens_producao', JSON.stringify(allOrdens));
  if (typeof globalThis.syncOrdensProducaoToSupabase === 'function')
    globalThis.syncOrdensProducaoToSupabase(allOrdens);
}
function saveMovInsumos() {
  localStorage.setItem('crm_insumo_movs', JSON.stringify(allMovInsumos));
}
function saveMovimentosEstoque() {
  localStorage.setItem('crm_movimentos_estoque', JSON.stringify(allMovimentosEstoque));
}
function saveReceitasProdutos() {
  ...
  if (typeof globalThis.syncReceitasToSupabase === 'function') {
    try {
      Promise.resolve(globalThis.syncReceitasToSupabase(allReceitasProdutos))
        .then(function () {
...
function getRecipeLinesForProduct(produtoId) {
  ...
  var lines = (allReceitasProdutos || []).filter(function (r) {
    return r && String(r.produto_id || '') === pid;
  });
  ...
  (allInsumos || []).forEach(function (i) {
    normalizeInsumo(i);
    byName[String(i.nome || '').toLowerCase()] = i;
  });
  return Object.entries(fallback.insumos)
    .map(function (e) {
      var nome = String(e[0] || '');
...
function buildAvailableStockMap() {
  ...
  allInsumos.forEach(function (i) {
    normalizeInsumo(i);
    byName[String(i.id)] = Number(i.estoque_atual) || 0;
  });
...
function computeInsumoOperationalTable() {
  ...
  return allInsumos
    .map(function (i) {
      ...
      var severity =
        (status === 'zerado' ? 100 : status === 'critico' ? 80 : status === 'baixo' ? 55 : 15) +
        (faltaOP > 0 ? 40 : 0) +
        (comprar > 0 && status !== 'ok' ? 15 : 0);
...
function renderProdKpis() {
  var el = document.getElementById('prod-kpis');
  if (!el) return;
  var ok = allInsumos.filter(function (i) {
    return getEstStatus(i) === 'ok';
  }).length;
  var atencao = allInsumos.filter(function (i) {
    return getEstStatus(i) === 'baixo';
  }).length;
  var valorTotal = allInsumos.reduce(function (s, i) {
    return s + (i.estoque || 0) * i.custo;
  }, 0);
  var ordsAtivas = allOrdens.filter(function (o) {
    return String(o.status || '') === 'em_producao';
  }).length;
  ...
  el.innerHTML =
    ...
    kpiCard(
      'Baixo estoque',
      atencao,
      atencao > 0 ? 'repor estoque' : 'tudo ok',
      atencao > 0 ? 'var(--red)' : 'var(--green)',
    ) +
    kpiCard(
      'Falta p/ OPs',
      fmtBRL(faltaCusto),
      open.length ? 'ordens abertas' : '—',
      'var(--red)',
    ) +
...
function renderInsumos() {
  var el = document.getElementById('insumos-list');
  if (!el) return;
  ...
  if (!list.length) {
    el.innerHTML = '<div class="empty">Nenhum insumo encontrado</div>';
    return;
  }
    ...
    list
      .map(function (i) {
        ...
        return (
          ...
          rowClass +
          '">' +
          '<td><span style="font-weight:800;color:var(--text)">' +
          escapeHTML(i.nome || '—') +
          '</span></td>' +
          ...
          '<button class="opp-mini-btn" onclick="abrirModalEntradaInsumo(\'' +
          safeId +
          '\')">Entrada</button> ' +
          '<button class="opp-mini-btn" onclick="abrirModalInsumo(\'' +
          safeId +
          '\')">Editar</button>' +
          '</td>' +
          '</tr>'
...
function renderOrdens() {
  var el = document.getElementById('ordens-prod-list');
  if (!el) return;
  ...
  var stBadge = function (st) {
    ...
    if (s === 'em_producao')
      return '<span class="chiva-badge chiva-badge-amber">em produção</span>';
  ...
  var list = []
    ...
    .map(function (o) {
      ...
      obj.custo_total_lote =
        obj.custo_total_lote == null ? null : Number(obj.custo_total_lote || 0) || 0;
  ...
  var toolbar =
    ...
    '<input id="search-ordem" class="filter-inp" placeholder="🔍 Buscar por lote/produto..." value="' +
    escapeHTML(q) +
    '" oninput="renderOrdens()" style="flex:1;min-width:180px"/>' +
    '<select id="fil-ordem-status" class="filter-sel" onchange="renderOrdens()">' +
    '<option value="">Todos status</option>' +
    '<option value="planejada" ' +
    (sf === 'planejada' ? 'selected' : '') +
    '>Planejada</option>' +
    '<option value="em_producao" ' +
    (sf === 'em_producao' ? 'selected' : '') +
    '>Em produção</option>' +
    '<option value="concluida" ' +
    (sf === 'concluida' ? 'selected' : '') +
    '>Concluída</option>' +
    '</select>' +
  ...
  if (!list.length) {
    el.innerHTML =
      toolbar + '<div class="empty" style="padding:40px 0">Nenhuma ordem de produção.</div>';
    return;
  }
  ...
  el.innerHTML =
    ...
    list
      .map(function (o) {
        ...
        var custo =
          o.custo_total_lote != null && Number(o.custo_total_lote || 0) > 0
            ? fmtBRL(Number(o.custo_total_lote || 0) || 0)
            : '—';
        return (
          '<tr>' +
          '<td class="chiva-table-mono">' +
          escapeHTML(o.lote || '—') +
          '</td>' +
          '<td><span style="font-weight:800;color:var(--text)">' +
          escapeHTML(o.produto_id || '—') +
          '</span></td>' +
          '<td style="text-align:right" class="chiva-table-mono">' +
          escapeHTML(fmtNum(o.quantidade_planejada, 2)) +
          '</td>' +
          '<td style="text-align:right" class="chiva-table-mono">' +
          escapeHTML(fmtNum(o.quantidade_produzida, 2)) +
          '</td>' +
          '<td style="text-align:right" class="chiva-table-mono">' +
          escapeHTML(custo) +
          '</td>' +
          '<td>' +
          escapeHTML(
            o.data_producao ? fmtDateBrFromIso(String(o.data_producao).slice(0, 10)) : '—',
          ) +
          '</td>' +
          '<td>' +
          stBadge(o.status) +
          ...
          '<td style="text-align:right;white-space:nowrap">' +
          (canStart
            ? '<button class="opp-mini-btn" onclick="setOrdemStatusQuick(\'' +
              safeId +
              "','em_producao')\">Iniciar</button> "
            : '') +
          (canFinish
            ? '<button class="opp-mini-btn" onclick="marcarOrdemConcluida(\'' +
              safeId +
              '\')">Concluir</button> '
            : '') +
          '<button class="opp-mini-btn" onclick="abrirModalPedidoProd(\'' +
          safeId +
          '\')">Editar</button>' +
          '</td>' +
          '</tr>'
...
function exportOrdensCustoCSV() {
  ...
  if (!list.length) {
    toast('Nenhuma ordem para exportar');
    return;
  }
  var esc = function (v) {
...
function renderMovimentosEstoque() {
  var el = document.getElementById('movimentos-estoque-list');
  if (!el) return;
  try {
    allMovimentosEstoque =
      safeJsonParse('crm_movimentos_estoque', null) || allMovimentosEstoque || [];
  } catch (_e) {}
  ...
  var toolbar =
    '<div class="prod-ordem-toolbar">' +
    '<input id="mov-search" class="filter-inp" placeholder="🔍 Lote, produto, insumo..." value="' +
    escapeHTML(movFilters.q || '') +
    '" oninput="renderMovimentosEstoque()" style="flex:1;min-width:180px"/>' +
    '<select id="mov-tipo" class="filter-sel" onchange="renderMovimentosEstoque()">' +
    tipoOpts +
    '</select>' +
    '<input id="mov-date-from" class="filter-inp" type="date" value="' +
    escapeHTML(movFilters.from || '') +
    '" onchange="renderMovimentosEstoque()" style="min-width:140px"/>' +
    '<input id="mov-date-to" class="filter-inp" type="date" value="' +
    escapeHTML(movFilters.to || '') +
    '" onchange="renderMovimentosEstoque()" style="min-width:140px"/>' +
    '</div>';

  if (!list.length) {
    el.innerHTML =
      toolbar + '<div class="empty" style="padding:40px 0">Nenhuma movimentação encontrada.</div>';
    return;
  }
    ...
    list
      .map(function (m) {
        return (
          ...
          '</td>' +
          '<td>' +
          escapeHTML(m.produto_id || '—') +
          '</td>' +
          '<td><span style="font-weight:800;color:var(--text)">' +
          escapeHTML(m.insumo_nome || '—') +
          '</span></td>' +
          '<td style="text-align:right" class="chiva-table-mono">' +
          escapeHTML(fmtNum(m.quantidade, 3) + ' ' + (m.unidade || '')) +
          '</td>' +
          '<td style="text-align:right" class="chiva-table-mono">' +
          escapeHTML(m.solicitado ? fmtNum(m.solicitado, 3) : '—') +
          '</td>' +
          '<td style="text-align:right" class="chiva-table-mono">' +
          escapeHTML(m.consumido ? fmtNum(m.consumido, 3) : '—') +
          '</td>' +
          '<td style="text-align:right" class="chiva-table-mono">' +
          escapeHTML(m.falta ? fmtNum(m.falta, 3) : '—') +
          '</td>' +
          '</tr>'
        );
      })
...
function renderSimuladorInputs() {
  var el = document.getElementById('sim-produtos-inputs');
  if (!el) return;
  el.innerHTML = Object.keys(RECEITAS_REAIS)
    .map(function (prod) {
      var r = RECEITAS_REAIS[prod];
      var cor = CAT_COLOR[r.categoria] || '#6bbf3a';
      return (
        '<div style="background:var(--card);border:1px solid var(--border);border-radius:var(--r-md);padding:var(--sp-3)">' +
...
function calcularSimulador() {
  ...
  if (!Object.keys(pedido).length) {
    document.getElementById('sim-resultado').innerHTML =
      '<div class="empty">Informe ao menos uma quantidade.</div>';
    return;
  }
  var nec = {};
  Object.entries(pedido).forEach(function (pe) {
    var r = RECEITAS_REAIS[pe[0]];
    if (!r) return;
    Object.entries(r.insumos).forEach(function (ie) {
      nec[ie[0]] = (nec[ie[0]] || 0) + (ie[1] * pe[1]) / 1000;
    });
  });
  var totalFalta = 0,
    totalTudo = 0;
  var rows = Object.entries(nec)
    .sort(function (a, b) {
      return b[1] - a[1];
    })
    .map(function (e) {
      var nome = e[0],
        nKg = e[1];
      var ins = allInsumos.find(function (i) {
        return i.nome === nome;
      });
      var disp = ins ? ins.estoque || 0 : 0;
      var falta = Math.max(0, nKg - disp);
      var unitCost =
        ins && ins.custo_unitario != null
          ? Number(ins.custo_unitario) || 0
          : PRECO_KG_FALLBACK[nome] || 0;
      ...
      var ok = falta < 0.001;
      return (
        '<div style="display:grid;grid-template-columns:1.5fr 75px 75px 75px 95px 90px;gap:8px;align-items:center;padding:8px 0;border-bottom:1px solid var(--border-sub)">' +
        '<span style="font-size:11px;font-weight:600">' +
        nome +
        '</span>' +
        '<span style="text-align:right;font-size:11px;font-family:var(--mono);color:var(--text-2)">' +
        nKg.toFixed(3) +
        ' kg</span>' +
        '<span style="text-align:right;font-size:11px;font-family:var(--mono);color:' +
        (disp > 0 ? 'var(--text)' : 'var(--red)') +
        '">' +
        disp.toFixed(3) +
        ' kg</span>' +
        '<span style="text-align:right;font-size:11px;font-family:var(--mono);font-weight:700;color:' +
        (ok ? 'var(--green)' : 'var(--red)') +
        '">' +
        (!ok ? falta.toFixed(3) + ' kg' : '—') +
        '</span>' +
        '<span style="text-align:right;font-size:11px;font-weight:700;color:' +
        (ok ? 'var(--text-3)' : 'var(--red)') +
        '">' +
        (!ok ? 'R$' + cf.toFixed(2) : 'R$0') +
        '</span>' +
        '<span style="text-align:right"><span style="padding:2px 8px;border-radius:9999px;font-size:9px;font-weight:800;background:' +
        (ok ? 'var(--green-bg)' : 'var(--red-bg)') +
        ';color:' +
        (ok ? 'var(--green)' : 'var(--red)') +
        '">' +
        (ok ? 'OK' : 'Comprar') +
  ...
  var resumo = Object.entries(pedido)
    .map(function (pe) {
      var r = RECEITAS_REAIS[pe[0]];
      return (
        '<span style="padding:4px 10px;border-radius:9999px;font-size:10px;font-weight:700;background:' +
        CAT_COLOR[r.categoria] +
        '20;color:' +
        CAT_COLOR[r.categoria] +
        ';border:1px solid ' +
        CAT_COLOR[r.categoria] +
  ...
  document.getElementById('sim-resultado').innerHTML =
    ...
    '<div style="font-size:22px;font-weight:800;font-family:var(--mono);color:var(--indigo-hi)">R$' +
    totalTudo.toFixed(2) +
    '</div>' +
    '</div>' +
    '<div style="background:var(--bg);border-radius:var(--r-md);padding:12px;text-align:center">' +
    ...
    (totalFalta > 0 ? 'var(--red)' : 'var(--green)') +
    '">' +
    (totalFalta > 0 ? 'R$' + totalFalta.toFixed(2) : 'OK') +
    '</div>' +
    '</div>' +
    '</div>' +
    '</div>' +
...
function salvarOrdemDeSimulacao() {
  ...
  var itens = Object.entries(window._ultimaSimulacao.pedido).map(function (e) {
    return { produto: e[0], qty: e[1] };
  });
...
function renderReceitaDetalhe() {
  ...
  products = products.filter(Boolean).sort(function (a, b) {
  ...
  console.log(
    'receitas carregadas:',
    Array.isArray(allReceitasProdutos) ? allReceitasProdutos.length : 0,
  );
  console.log('produtos no select:', products.length);
  if (!prod) {
    el.innerHTML = '<div class="empty">Selecione um produto para ver a ficha técnica.</div>';
    return;
  }
  ...
  var hasSaved = (allReceitasProdutos || []).some(function (rp) {
    return rp && String(rp.produto_id || '') === String(prod);
  });
  if (!hasSaved) {
  ...
  var idxInsumoById = {};
  insumosSorted.forEach(function (i) {
    idxInsumoById[String(i.id)] = i;
  });
  rows.forEach(function (rp) {
      ...
      ? rows
          .map(function (rp) {
            ...
            return (
              ...
              escapeHTML(rid) +
              '">' +
              '<td><select class="chiva-select" style="width:100%" data-f="insumo">' +
              selOps +
              '</select></td>' +
              '<td style="text-align:right"><input class="chiva-input" data-f="qty" type="number" step="0.01" style="max-width:160px;display:inline-block" value="' +
              escapeHTML(qty) +
              '"/></td>' +
              ...
              (unit === 'unidade' ? ' selected' : '') +
              '>unidade</option>' +
              '</select></td>' +
              '<td style="text-align:right"><button class="opp-mini-btn" onclick="removerIngredienteReceita(\'' +
              escapeJsSingleQuote(rid) +
              '\')">Remover</button></td>' +
              '</tr>'
            );
          })
          .join('')
      : '<tr><td colspan="4" style="color:var(--text-3)">Nenhum ingrediente ainda. Clique em “+ Ingrediente”.</td></tr>') +
    '</tbody>' +
    '</table>' +
    '</div>';
...
function adicionarIngredienteReceita() {
  ...
  if (!first) {
    toast('Cadastre insumos primeiro');
    return;
  }
...
function removerIngredienteReceita(id) {
  var rid = String(id || '');
  allReceitasProdutos = allReceitasProdutos.filter(function (rp) {
    return String(rp.id || '') !== rid;
  });
  saveReceitasProdutos();
  renderReceitaDetalhe();
}
...
function salvarReceitaProduto() {
  ...
  if (!isUuidLike(prod)) {
    toast('⚠ Produto inválido. Selecione um produto válido.');
    return;
  }
  ...
  trs.forEach(function (tr) {
    ...
    var rp = allReceitasProdutos.find(function (x) {
      return String(x.id || '') === rid;
    });
    if (!rp) return;
    rp.produto_id = prod;
    rp.insumo_id = String(ins ? ins.value : '').trim();
    if (!rp.insumo_id || !isUuidLike(rp.insumo_id)) {
      toRemoveIds.push(rid);
      return;
    }
    var rawQty = String(qtyEl ? qtyEl.value : '')
      .trim()
      .replace(',', '.');
  ...
  if (toRemoveIds.length) {
    allReceitasProdutos = allReceitasProdutos.filter(function (rp) {
      return toRemoveIds.indexOf(String(rp.id || '')) < 0;
    });
  }
  allReceitasProdutos.forEach(function (rp) {
...
function abrirModalInsumo(id) {
  ...
  if (id) {
    var i = allInsumos.find(function (x) {
    ...
    ['in-nome', 'in-estoque', 'in-minimo', 'in-custo', 'in-fornecedor', 'in-lead-time'].forEach(
      function (x) {
...
function abrirModalEntradaInsumo(id) {
  ...
  document.getElementById('modal-entrada-insumo-title').textContent =
    'Registrar Entrada: ' + i.nome;
...
function registrarEntradaInsumo() {
  ...
  var data =
    parseDateToIso(String(document.getElementById('en-data').value || '')) ||
...
function abrirModalPedidoProd(id) {
  ...
  try {
    allReceitasProdutos = safeJsonParse('crm_receitas_produtos', null) || allReceitasProdutos || [];
    allProdutosReceitas =
      safeJsonParse('crm_receitas_produtos_produtos', null) || allProdutosReceitas || [];
  } catch (_e) {}
  ...
  document.getElementById('or-data').value = fmtDateBrFromIso(
    o ? String(o.data_producao || today) : today,
  );
...
function salvarOrdem() {
  ...
  var date =
    parseDateToIso(String(document.getElementById('or-data').value || '')) ||
    new Date().toISOString().slice(0, 10);
  ...
  if (qp <= 0) {
    toast('Informe a quantidade planejada');
    return;
  }
  ...
  var savedId = id;
  if (id) {
    var idx = allOrdens.findIndex(function (x) {
...
function setOrdemStatusQuick(id, status) {
  var o = allOrdens.find(function (x) {
...
function calcularConsumoDaOP(o) {
  var units =
    Number((o && o.quantidade_produzida) || 0) > 0
      ? Number(o.quantidade_produzida || 0)
      : Number((o && o.quantidade_planejada) || 0) || 0;
...
function marcarBaixaRealizada(o, result) {
  ...
  if (result && result.custo_total_lote != null)
    tag += ' CUSTO=' + Number(result.custo_total_lote || 0).toFixed(2);
  o.observacoes =
    String(o.observacoes || '').trim() + (String(o.observacoes || '').trim() ? '\n' : '') + tag;
}
...
function baixarEstoqueDaOrdem(id, opts) {
  ...
  if (!opts.silent && !confirm('Baixar estoque desta OP agora?'))
...
function setProdTab(tab) {
  ...
  if (tab === 'simulador') {
    ...
    var sr = document.getElementById('sim-resultado');
    if (sr) sr.innerHTML = '';
  }
