...
// safeJsonParse importado de ../_shared/utils.ts
...
async function getBlingLojaIdMap(
  supabaseUrl: string,
  serviceRoleKey: string,
...
function inferCanalSlugFromBling(detailData: any, lojaIdMap: Record<string, string>) {
  ...
  const lojaNome = String(
    lojaObj?.nome ?? lojaObj?.descricao ?? data?.lojaNome ?? data?.nomeLoja ?? '',
  ).trim();

  const canalVendaObj = data?.canalVenda ?? data?.canal_venda ?? {};
  const origemObj =
    data?.origem ?? data?.origemVenda ?? data?.marketplace ?? data?.market_place ?? {};
  ...
  const numeroPedidoEcommerce = String(
    data?.numeroPedidoEcommerce ??
      data?.numeroPedidoLoja ??
      data?.numeroPedidoEcommerceLoja ??
      data?.numeroPedidoLojaEcommerce ??
      '',
  ).trim();
  ...
  const guess = (text: string) => {
    ...
    if (!hay) return '';
    if (/\bmercado\s*livre\b|\bmercadolivre\b|\bmeli\b|\bmlb\b/.test(hay) || /^mlb/.test(hay))
      return 'ml';
  ...
  let slug = guess(hay);
  const mapped =
    lojaId && lojaIdMap[lojaId]
      ? String(lojaIdMap[lojaId] || '')
          .trim()
          .toLowerCase()
      : '';
  ...
  if (slug && mapped && slug !== mapped) {
    if (slug !== 'yampi') {
      return {
        ...
        lojaId,
        lojaNome,
        canalNome: origemNome || canalNome || canalVendaNome || ecommerceNome,
        numeroPedidoEcommerce,
      };
    }
  }
  ...
  return {
    ...
    lojaId,
    lojaNome,
    canalNome: origemNome || canalVendaNome || ecommerceNome || canalNome,
    numeroPedidoEcommerce,
  };
}
...
async function ensureCanaisAndGetMap(
  supabaseUrl: string,
  serviceRoleKey: string,
): Promise<Record<string, string>> {
  const supabase = createClient(supabaseUrl, serviceRoleKey);
...
async function persistSyncResultToDb(
  ...
  orders.forEach((o) => {
    ...
    const uf = String(end?.uf ?? o?.uf_entrega ?? '')
      .trim()
      .toUpperCase();
  ...
  if (updateLastSync) {
    await supabase
      .from('configuracoes')
      .upsert([{ chave: 'ultima_sync_bling', valor_texto: now, updated_at: now }], {
        onConflict: 'chave',
      });
  }
}
...
function mapBlingOrder(detail: any) {
  ...
  const itens = Array.isArray(itensRaw)
    ? itensRaw
        .map((it) => {
          ...
          const quantidade =
            Number(node?.quantidade ?? it?.quantidade ?? node?.qty ?? it?.qty ?? 0) || 0;
          const valor =
            Number(
              node?.valor ??
                it?.valor ??
  ...
  const numero = String(
    data?.numero ?? data?.numeroPedido ?? data?.numeroPedidoLoja ?? data?.id ?? '',
  ).trim();
  ...
  const doc = onlyDigits(
    contato?.numeroDocumento ?? contato?.cpfCnpj ?? contato?.cpf ?? contato?.cnpj ?? '',
  );
  ...
  try {
    ...
    if (backfillOrigins) {
      ...
      const { data: rows, error } = await supabase
        ...
        .eq('source', 'bling')
        .or(
          'origem_canal.is.null,origem_canal.eq.,origem_canal_nome.is.null,tipo_venda.is.null,canal_id.is.null',
        )
    ...
    if (reprocessExisting) {
      ...
      if (!pedidoIdsForApi.length) {
        try {
          await supabase
            .from('configuracoes')
            .upsert([{ chave: cursorKey, valor_texto: '', updated_at: nowIso() }], {
              onConflict: 'chave',
            });
        } catch (_e) {}
        return jsonResponse({
    ...
    if (persist) {
      ...
      if (!hasMore && syncProducts) {
        try {
          const count = await syncBlingProductsToSupabase(
            tokenRef,
            supabaseUrl,
            serviceRoleKey,
            prodLimit,
