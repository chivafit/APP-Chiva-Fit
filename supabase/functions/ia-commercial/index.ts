...
serve(async (req: Request) => {
  ...
  if (supabaseUrl && serviceRoleKey) {
    const rl = await checkRateLimit(supabaseUrl, serviceRoleKey, userEmail, "ia-commercial", MAX_CALLS_PER_HOUR);
    if (!rl.allowed) {
      return jsonResponse(
  ...
  const system = "Você é um assistente comercial especializado em CRM para e-commerce. " +
