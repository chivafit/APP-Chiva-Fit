...
export function scheduleAutoBlingSync(ctx) {
  ...
  blingAutoSyncTimer = setInterval(
    () => {
      maybeRunAutoBlingSync(ctx).catch((e) => {
        ...
        try {
          ctx.toast('⚠ Sync automático Bling falhou: ' + msg, 'error');
        } catch (_e) {}
      });
    },
    20 * 60 * 1000,
  ...
  maybeRunAutoBlingSync(ctx).catch((e) => {
    ...
    try {
      ctx.toast('⚠ Sync inicial Bling falhou: ' + msg, 'error');
    } catch (_e) {}
  });
}
...
export async function syncBling(ctx, options) {
  ...
  try {
    ...
    const setStatus = (html, ok) => {
      ...
      st.innerHTML = html;
      st.className =
        ok === true ? 'setup-status s-ok' : ok === false ? 'setup-status s-err' : 'setup-status';
    };
    ...
    while (hasMore) {
      ...
      const payload = omitDates
        ? { limit: batchLimit, offset }
        : { from, to, limit: batchLimit, offset };
      ...
      if (batch > 400)
        throw new Error(
          'Importação interrompida (muitos lotes). Ajuste o período ou aumente o limit.',
        );
    }
...
export async function syncBlingProdutos(ctx) {
  ...
  if (st) {
    st.textContent = 'Importando produtos...';
  ...
  try {
    ...
    if (!resp.ok) {
      const msg =
        data && (data.message || data.error)
          ? String(data.message || data.error)
          : txt || 'Erro na função Bling (produtos)';
      throw new Error(msg);
    }
    ...
    if (!products.length) {
      if (st) {
        st.textContent = '⚠ Nenhum produto retornado do Bling.';
        st.className = 'setup-status s-err';
      }
      return;
    }
    ...
    if (document.getElementById('page-produtos')?.classList.contains('active'))
      ctx.renderProdutos();
    if (st) {
      st.textContent = `✓ ${rows.length} produtos importados`;
      st.className = 'setup-status s-ok';
    }
    try {
      if (ctx.isSupaReady())
    ...
    if (st) {
      const hint = /relation .*v2_produtos.*does not exist|42P01/i.test(msg)
...
export async function backfillBlingEnderecos(ctx) {
  const st = document.getElementById('bling-status');
  if (st) {
    st.textContent = 'Backfill: buscando período no Supabase...';
    st.className = 'setup-status';
  }
  try {
    if (!ctx?.isSupaReady?.()) throw new Error('Conecte o Supabase primeiro.');
    ...
    if (st) {
      st.textContent = `Backfill: importando Bling (${from} → ${to})...`;
      st.className = 'setup-status';
    }
    ...
    if (!orders.length) {
      if (st) {
        st.textContent = 'Backfill: nenhum pedido retornado.';
        st.className = 'setup-status s-err';
      }
      return;
    }
    ...
    if (st) {
      st.textContent = `✓ Backfill concluído: ${orders.length} pedidos`;
      st.className = 'setup-status s-ok';
    }
