export async function syncYampi(ctx) {
  ...
  if (st) {
    st.textContent = 'Sincronizando com Supabase...';
...
export async function syncCarrinhosAbandonadosYampi(ctx) {
  ...
  if (st) {
    st.textContent = 'Sincronizando carrinhos...';
    st.className = 'setup-status';
  }
  try {
    const raw = await ctx.fetchYampiAbandoned();
    const next = (Array.isArray(raw) ? raw : [])
      .map(ctx.normalizeCarrinhoAbandonado)
      .filter((c) => c.checkout_id);
    const byId = {};
    (ctx.getCarrinhosAbandonados() || []).forEach((c) => {
      if (c && c.checkout_id) byId[String(c.checkout_id)] = c;
    });
    next.forEach((c) => {
      const prev = byId[c.checkout_id] || null;
      byId[c.checkout_id] = prev
        ? {
            ...prev,
            ...c,
            recuperado: prev.recuperado || c.recuperado,
            recuperado_em: prev.recuperado_em || c.recuperado_em,
            recuperado_pedido_id: prev.recuperado_pedido_id || c.recuperado_pedido_id,
          }
        : c;
    });
    ...
    if (st) {
      st.textContent = `✓ ${merged.length} carrinhos carregados`;
      st.className = 'setup-status s-ok';
    }
