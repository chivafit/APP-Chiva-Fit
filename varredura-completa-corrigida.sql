-- ============================================================
-- VARREDURA COMPLETA - MÓDULO RECOMPRA & WHATSAPP (CORRIGIDA)
-- ============================================================
...
-- Segmentos (corrigido - sem join)
SELECT 
  'SEGMENTOS' as tipo,
  nome,
  descricao,
  automatico,
  CASE WHEN automatico THEN '✅ Auto' ELSE '📝 Manual' END as tipo_segmento
FROM customer_segments
ORDER BY automatico DESC, nome;
...
-- Clientes elegíveis para cada regra ativa
SELECT 
  ar.nome as regra,
  COUNT(*) as elegiveis,
  COUNT(CASE WHEN c.total_pedidos > 0 THEN 1 END) as com_pedidos,
  COUNT(CASE WHEN c.ultimo_pedido < NOW() - INTERVAL '30 days' THEN 1 END) as inativos_30d
FROM automation_rules ar
CROSS JOIN LATERAL fn_eligible_for_rule(ar.id, 100) eleg
LEFT JOIN LATERAL (SELECT * FROM v2_clientes WHERE id = eleg.cliente_id) c ON true
WHERE ar.ativo = TRUE
