-- ============================================================
-- VARREDURA COMPLETA - MÓDULO RECOMPRA & WHATSAPP
-- ============================================================
...
SELECT 
  'SEGMENTOS' as tipo,
  cs.nome as segmento,
  COUNT(cs.cliente_id) as total_clientes
FROM customer_segments cs
LEFT JOIN customer_segment_clients csc ON csc.segment_id = cs.id
...
SELECT 
  ...
  SUM(total_lidos) as total_lidos,
  SUM(total_convertidos) as total_convertidos,
  ROUND(SUM(CASE WHEN total_enviados > 0 THEN (total_convertidos::float / total_enviados) * 100 ELSE 0 END), 2) as taxa_conversao
...
SELECT 
  ...
  COUNT(CASE WHEN data_criacao > NOW() - INTERVAL '30 days' THEN 1 END) as ultimos_30_dias
...
SELECT 
  ...
  ROUND(COUNT(CASE WHEN convertido = TRUE THEN 1 END)::float / NULLIF(COUNT(*), 0) * 100, 2) as taxa_conversao_runs
...
SELECT 
  ...
  AVG(COALESCE(receita, 0)) as receita_media
